...
The cybervinci server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@cybervinci-ai/llm` tool loop after replacing its
...
- cybervinci instance has stuff like `cybervinci.session.prompt()` or
  `cybervinci.tool.register({...})`
...
- decide whether to preserve deprecated `@cybervinci-ai/llm` orchestration exports
