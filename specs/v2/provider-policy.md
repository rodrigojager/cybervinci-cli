...
Both `action` and `resource` use cybervinci's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.cybervinci` files is intentionally deferred until `.cybervinci` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether cybervinci permits an operation through its managed execution paths.
