...
Move the canonical CYBERVINCI terminal application from
`packages/cybervinci/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/cybervinci ---\
                      > @cybervinci-ai/tui -> @cybervinci-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/cybervinci`,
`packages/cli`, or `@cybervinci-ai/core`.

The SDK is the TUI's CYBERVINCI boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/cybervinci`.
- Do not replace private `packages/cybervinci` imports with `@cybervinci-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@cybervinci-ai/tui`.

## Ownership Boundary

### `@cybervinci-ai/tui` Owns
...
- CYBERVINCI domain data displayed by the TUI
...
```text
packages/cybervinci/src/cli/cmd/tui
```

Its private dependency on `packages/cybervinci` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/cybervinci/src/*`.
...
Status: Completed. The private `@cybervinci-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@cybervinci-ai/tui`.
...
- The package has no dependency on `cybervinci`, `@cybervinci-ai/cli`, or
  `@cybervinci-ai/core`.
...
primitives now live in `@cybervinci-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@cybervinci-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads CYBERVINCI globals or process state;
...
Replace process-global CYBERVINCI state with resolved TUI inputs.
...
`@cybervinci-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/cybervinci` initially.
...
canonical in `@cybervinci-ai/tui`. Configured references resolve through the new
...
adapters rather than CYBERVINCI domain implementations.

Make the SDK the only CYBERVINCI domain boundary used by the TUI.
...
- Domain-facing TUI code imports CYBERVINCI data and operations only from
  `@cybervinci-ai/sdk`.
...
`@cybervinci-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/cybervinci`.
...
- The package has no imports from `packages/cybervinci`, `packages/cli`, or
  `@cybervinci-ai/core`.
...
public `@cybervinci-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/cybervinci`.
- Keep the legacy embedded worker and server startup in `packages/cybervinci`.
- Change those adapters to load config, create transport inputs, and call the
  public `@cybervinci-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/cybervinci`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@cybervinci-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@cybervinci-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for CYBERVINCI domain state.
...
cd packages/cybervinci && bun typecheck
...
rg '@cybervinci-ai/core|packages/cybervinci|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/cybervinci/src packages/cybervinci/test
```
...
- Build the current-platform `packages/cybervinci` binary.
