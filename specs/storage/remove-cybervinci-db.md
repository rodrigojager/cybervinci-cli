# Remove `packages/cybervinci/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/cybervinci/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the cybervinci legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/cybervinci/src/storage/db.ts` are concentrated in 22 source files:

- `packages/cybervinci/src/account/repo.ts`
- `packages/cybervinci/src/cli/cmd/db.ts`
- `packages/cybervinci/src/cli/cmd/import.ts`
- `packages/cybervinci/src/cli/cmd/stats.ts`
- `packages/cybervinci/src/control-plane/workspace.ts`
- `packages/cybervinci/src/index.ts`
- `packages/cybervinci/src/node.ts`
- `packages/cybervinci/src/permission/index.ts`
- `packages/cybervinci/src/project/project.ts`
- `packages/cybervinci/src/server/projectors.ts`
- `packages/cybervinci/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/cybervinci/src/server/shared/fence.ts`
- `packages/cybervinci/src/session/message-v2.ts`
- `packages/cybervinci/src/session/projectors.ts`
- `packages/cybervinci/src/session/prompt.ts`
- `packages/cybervinci/src/session/session.ts`
- `packages/cybervinci/src/session/todo.ts`
- `packages/cybervinci/src/share/share-next.ts`
- `packages/cybervinci/src/storage/db.ts`
- `packages/cybervinci/src/sync/index.ts`
- `packages/cybervinci/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy cybervinci database wrapper; `packages/cybervinci/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the cybervinci wrapper.
...
Status: Completed. `SyncEvent` and the cybervinci projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy cybervinci database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy cybervinci database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy cybervinci database wrapper.
...
All migration groups are complete or superseded. `packages/cybervinci/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No cybervinci data-migration group remains.

The previous cybervinci `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate cybervinci data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/cybervinci`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/cybervinci/src`
- `bun typecheck` from `packages/cybervinci`
- Relevant package tests from `packages/cybervinci`, not the repo root
