...
import { PromptInput } from "@cybervinci-ai/schema/prompt-input"
import { Session } from "@cybervinci-ai/schema/session"
import { Project } from "@cybervinci-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@cybervinci-ai/schema/schema"
...
import { SessionEvent } from "@cybervinci-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this CYBERVINCI process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this CYBERVINCI process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
