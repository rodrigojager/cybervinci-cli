...
This is the living design and status document for `@cybervinci-ai/codemode` and its existing V2 CYBERVINCI adapter.
...
### V2 CYBERVINCI adapter

This section describes the `v2` branch integration. On `dev`, CodeMode is integrated through
`packages/cybervinci/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
