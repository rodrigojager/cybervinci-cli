...
- Author package entrypoint is `@cybervinci-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/cybervinci-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@cybervinci-ai/plugin/tui`.
- `@cybervinci-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running CYBERVINCI version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.cybervinci` is absent, no check is performed (backward compatible).
...
- `cybervinci plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `cybervinci plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.cybervinci` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.cybervinci`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.cybervinci`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.cybervinci` dir or `CYBERVINCI_CONFIG_DIR`, CYBERVINCI installs `@cybervinci-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@cybervinci-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus CYBERVINCI's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
CYBERVINCI registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, CYBERVINCI pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@cybervinci-ai/plugin/tui`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `cybervinci.default`.
...
- Local plugins persist installed themes under the local `.cybervinci/themes` area near the plugin config source.
...
- `--pure` / `CYBERVINCI_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.cybervinci/plugins/tui-smoke.tsx`
- Local vim plugin: `.cybervinci/plugins/tui-vim.tsx`
- Local smoke config: `.cybervinci/tui.json`
- Local smoke theme: `.cybervinci/plugins/smoke-theme.json`
