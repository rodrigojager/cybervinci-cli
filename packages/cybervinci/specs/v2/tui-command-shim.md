...
Delete `packages/cybervinci/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/cybervinci/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/cybervinci/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/cybervinci`
- TUI plugin loader tests in `packages/cybervinci` if runtime plugin API wiring changed
