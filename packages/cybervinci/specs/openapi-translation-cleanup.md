...
Trim `packages/cybervinci/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/cybervinci/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/cybervinci`.
- `[x]` `bun typecheck` from `packages/cybervinci`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/cybervinci/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/cybervinci`.
- `[x]` `bun dev generate > /tmp/cybervinci-openapi.json` from `packages/cybervinci`.
...
- Edit `packages/cybervinci/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/cybervinci-openapi.json` from `packages/cybervinci`.
...
- Check source schemas in files like `packages/cybervinci/src/session/schema.ts`, `packages/cybervinci/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/cybervinci/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/cybervinci/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
