...
Practical reference for a future `packages/server` split after the cybervinci
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/cybervinci`.
...
Until enough shared service code lives outside `packages/cybervinci`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/cybervinci`

It should not import `packages/cybervinci` services while `packages/cybervinci`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on cybervinci-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/cybervinci` implementation details.
