...
import { LLMEvent, LLMResponse } from "@cybervinci-ai/llm"
import { RequestExecutor } from "@cybervinci-ai/llm/route"
...
const selectedScenarios = new Set(
  (envValue("CYBERVINCI_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the cybervinci LLM stack remain real.
  const metadata = {
