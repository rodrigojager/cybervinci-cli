// Subprocess test harness for the cybervinci CLI. Spawns the real binary against
...
//
// Configuration flows through cybervinci's built-in test affordances:
//   - CYBERVINCI_CONFIG_CONTENT      : provider config inline, no files to find
//   - CYBERVINCI_TEST_HOME           : pins os.homedir() → tmpdir
//   - CYBERVINCI_DISABLE_PROJECT_CONFIG : skip walking up for cybervinci.json
//   - CYBERVINCI_PURE                : skip external plugin discovery + install
//   - CYBERVINCI_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `cybervinci.run` is fully wired. The shape supports adding more
// builders (`cybervinci.serve(opts)`, `cybervinci.acp(opts)`, `cybervinci.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of CyberVinciCli.
...
const cybervinciRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(cybervinciRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    CYBERVINCI_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    CYBERVINCI_CONFIG_CONTENT: configJson,
    CYBERVINCI_DISABLE_PROJECT_CONFIG: "1",
    CYBERVINCI_PURE: "1",
    CYBERVINCI_DISABLE_AUTOUPDATE: "1",
    CYBERVINCI_DISABLE_AUTOCOMPACT: "1",
    CYBERVINCI_DISABLE_MODELS_FETCH: "1",
...
// Typed equivalent of constructing argv for `cybervinci run`. New flags should
...
// `cybervinci serve` is a long-lived process — it never exits on its own.
...
// `cybervinci acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type CyberVinciCli = {
  ...
  // Spawn `cybervinci serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `cybervinci acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly cybervinci: CyberVinciCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("cybervinci.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          CYBERVINCI_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("cybervinci.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("cybervinci.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "cybervinci server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `cybervinci serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("cybervinci.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const cybervinci: CyberVinciCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, cybervinci })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `cybervinci.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
// every call site. Use this for any test that needs the cybervinci CLI fixture.
//
...
// resources (e.g. `cybervinci.serve`) without an extra `Effect.scoped` wrapper —
