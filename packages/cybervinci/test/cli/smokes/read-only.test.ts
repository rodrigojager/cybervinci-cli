...
describe("cybervinci read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // CYBERVINCI_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ cybervinci }) =>
      Effect.gen(function* () {
        const r = yield* cybervinci.spawn(["mcp", "list"])
        cybervinci.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ cybervinci }) =>
      Effect.gen(function* () {
        const r = yield* cybervinci.spawn(["providers", "list"])
        cybervinci.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ cybervinci }) =>
      Effect.gen(function* () {
        const r = yield* cybervinci.spawn(["models"])
        cybervinci.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ cybervinci }) =>
      Effect.gen(function* () {
        const r = yield* cybervinci.spawn(["agent", "list"])
        cybervinci.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh CYBERVINCI_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ cybervinci }) =>
      Effect.gen(function* () {
        const r = yield* cybervinci.spawn(["session", "list"])
        cybervinci.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ cybervinci }) =>
      Effect.gen(function* () {
        const r = yield* cybervinci.spawn(["stats"])
        cybervinci.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under CYBERVINCI_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ cybervinci }) =>
      Effect.gen(function* () {
        const r = yield* cybervinci.spawn(["db", "path"])
        cybervinci.expectExit(r, 0, "db path")
