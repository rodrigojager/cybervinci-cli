...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/cybervinci.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/cybervinci.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/cybervinci.jsonc", dir: ".cybervinci", suggestion: "opencode" },
        expected:
          'Directory ".cybervinci" in /tmp/cybervinci.jsonc is not valid. Rename the directory to "opencode" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/cybervinci.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/cybervinci.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `cybervinci models` to list available models",
      "Or check your config (cybervinci.json) provider/model names",
    ].join("\n")
