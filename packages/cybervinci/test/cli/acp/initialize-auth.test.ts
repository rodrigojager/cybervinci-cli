...
describe("cybervinci acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ cybervinci }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ cybervinci }))
        ...
        expect(initialized.agentInfo?.name).toBe("CYBERVINCI")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ cybervinci }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ cybervinci })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("cybervinci-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "cybervinci-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.CYBERVINCI_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ cybervinci }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("cybervinci-login")
