...
import { CyberVinciClient, type GlobalEvent } from "@cybervinci-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<CyberVinciClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<CyberVinciClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<CyberVinciClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<CyberVinciClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<CyberVinciClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://cybervinci.test"),
...
function sdk(
  input: {
    ...
    subscribe?: CyberVinciClient["event"]["subscribe"]
    globalEvent?: CyberVinciClient["global"]["event"]
    promptAsync?: CyberVinciClient["session"]["promptAsync"]
    status?: CyberVinciClient["session"]["status"]
    messages?: CyberVinciClient["session"]["messages"]
    children?: CyberVinciClient["session"]["children"]
    permissions?: CyberVinciClient["permission"]["list"]
    questions?: CyberVinciClient["question"]["list"]
  } = {},
) {
  const client = new CyberVinciClient()

  const subscribe: CyberVinciClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: CyberVinciClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: CyberVinciClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: CyberVinciClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: CyberVinciClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: CyberVinciClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: CyberVinciClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: CyberVinciClient["question"]["list"] = input.questions ?? (() => ok([]))
