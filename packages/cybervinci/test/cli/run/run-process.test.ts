// Subprocess integration tests for `cybervinci run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `cybervinci.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `CYBERVINCI_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("cybervinci run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, cybervinci }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* cybervinci.run("think", { extraArgs: ["--thinking"] })
        cybervinci.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* cybervinci.run("think again")
        cybervinci.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, cybervinci }) =>
      Effect.gen(function* () {
        ...
        const result = yield* cybervinci.run("say hi", { format: "json" })
        cybervinci.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ cybervinci }) =>
      Effect.gen(function* () {
        const result = yield* cybervinci.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, cybervinci }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* cybervinci.run("request permission", { permission: { bash: "ask" } })
        cybervinci.expectExit(denied, 0)
        ...
        const allowed = yield* cybervinci.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        cybervinci.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* cybervinci.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        cybervinci.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, cybervinci }) =>
      Effect.gen(function* () {
        ...
        const server = yield* cybervinci.serve()

        const result = yield* cybervinci.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, cybervinci }) =>
      Effect.gen(function* () {
        const result = yield* cybervinci.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, cybervinci }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* cybervinci.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
