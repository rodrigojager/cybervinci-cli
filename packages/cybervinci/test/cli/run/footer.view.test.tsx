...
import type { QuestionRequest } from "@cybervinci-ai/sdk/v2"
import { CyberVinciKeymapProvider, registerCyberVinciKeymap } from "@cybervinci-ai/tui/keymap"
import {
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerCyberVinciKeymap(keymap, renderer, config)
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerCyberVinciKeymap(keymap, renderer, tuiConfig)
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerCyberVinciKeymap(keymap, renderer, tuiConfig)
