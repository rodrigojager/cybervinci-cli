...
import { EditorContextProvider, useEditorContext, type EditorIntegration } from "@cybervinci-ai/tui/context/editor"
import { tmpdir } from "../../fixture/fixture"
import { FakeWebSocket } from "../../lib/websocket"
import { TestTuiContexts } from "../../fixture/tui-environment"
import { discoverEditorConnection } from "@cybervinci-ai/tui/editor"

const originalClaudePort = process.env.CLAUDE_CODE_SSE_PORT
const originalCyberVinciPort = process.env.CYBERVINCI_EDITOR_SSE_PORT

afterEach(() => {
  process.env.CLAUDE_CODE_SSE_PORT = originalClaudePort
  process.env.CYBERVINCI_EDITOR_SSE_PORT = originalCyberVinciPort
})
...
function mountEditorContext(WebSocketImpl?: typeof WebSocket) {
  ...
  createRoot((nextDispose) => {
    ...
    const value = process.env.CLAUDE_CODE_SSE_PORT || process.env.CYBERVINCI_EDITOR_SSE_PORT
    return (
...
test("useEditorContext reconnect switches editor server by session directory", async () => {
  ...
  process.env.CYBERVINCI_EDITOR_SSE_PORT = undefined
...
test("useEditorContext connects with CYBERVINCI_EDITOR_SSE_PORT", async () => {
  await using tmp = await tmpdir()
  process.env.CLAUDE_CODE_SSE_PORT = undefined
  process.env.CYBERVINCI_EDITOR_SSE_PORT = "4020"
