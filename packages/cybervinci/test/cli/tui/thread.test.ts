...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ cybervinci }) =>
    Effect.gen(function* () {
      const result = yield* cybervinci.spawn(["--replay-limit", "10"])

      cybervinci.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ cybervinci }) =>
    Effect.gen(function* () {
      const result = yield* cybervinci.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ cybervinci }) =>
    Effect.gen(function* () {
      const result = yield* cybervinci.spawn(["--mini", "--port", "4096"])
