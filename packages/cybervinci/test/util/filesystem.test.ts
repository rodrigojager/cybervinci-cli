...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "cybervinci.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "cybervinci.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "cybervinci.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "cybervinci.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["cybervinci.json", "cybervinci.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "cybervinci.json"),
        path.join(project, "cybervinci.jsonc"),
      ])
    })
  })
