...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".cybervinci")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .cybervinci/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const cybervinci = path.join(test.directory, ".cybervinci")
      const tool = path.join(cybervinci, "tool")
  ...
  it.instance("ignores non-tool exports in .cybervinci/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".cybervinci", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-cybervinci` was registering a tool
  ...
  it.instance("loads tools from .cybervinci/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const cybervinci = path.join(test.directory, ".cybervinci")
      const tools = path.join(cybervinci, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".cybervinci", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const cybervinci = path.join(test.directory, ".cybervinci")
        const customTools = path.join(cybervinci, "tools")
        const plugin = path.join(cybervinci, "node_modules", "@cybervinci-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(cybervinci, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@cybervinci-ai/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".cybervinci", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(cybervinci, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@cybervinci-ai/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(cybervinci, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(cybervinci, "node_modules", "cowsay")
