...
import { createCyberVinciClient } from "@cybervinci-ai/sdk/v2"
...
type Sdk = ReturnType<typeof createCyberVinciClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.CYBERVINCI_SERVER_PASSWORD = input?.password
      Flag.CYBERVINCI_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".cybervinci", "skills", "project-rest-skill", "SKILL.md"),
      `---
