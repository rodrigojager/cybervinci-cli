...
const testStateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      ...
      envPassword: process.env.CYBERVINCI_SERVER_PASSWORD,
      envUsername: process.env.CYBERVINCI_SERVER_USERNAME,
    }
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("CYBERVINCI_SERVER_PASSWORD", original.envPassword)
        restoreEnv("CYBERVINCI_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
...
function authConfigLayer(input?: { password?: string; username?: string }) {
  return ServerAuth.Config.configLayer({
    ...
    username: input?.username ?? "cybervinci",
  })
}
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            CYBERVINCI_SERVER_PASSWORD: input?.password,
            CYBERVINCI_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
      ...
      Effect.gen(function* () {
        ...
        yield* router.add("*", "/*", (request) =>
          serveUIEffect(request, {
            ...
            uiUpstream: new URL("https://ui.cybervinci.invalid"),
          }),
        )
      }),
    ).pipe(
...
describe("HttpApi UI fallback", () => {
  it.live("serves the web UI through the HTTP API app", () =>
    Effect.gen(function* () {
      ...
      const response = yield* uiApp({
        ...
        client: httpClient(
          new Response("<html>cybervinci</html>", { headers: { "content-type": "text/html" } }),
          (request) => {
            proxiedUrl = request.url
          },
        ),
      ...
      expect(yield* responseText(response)).toBe("<html>cybervinci</html>")
      expect(proxiedUrl).toBe("https://ui.cybervinci.invalid/")
    }),
  )
  ...
  it.live("strips upstream transfer encoding headers from proxied assets", () =>
    Effect.gen(function* () {
      ...
      expect(response.status).toBe(200)
      expect(proxiedUrl).toBe("https://ui.cybervinci.invalid/assets/app.js")
            ...
            Layer.succeed(
              ...
              HttpClient.make((request) =>
                Effect.succeed(
                  HttpClientResponse.fromWeb(
                    request,
                    new Response("<html>cybervinci</html>", {
                      headers: {
  ...
  it.live("accepts auth token for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        username: "cybervinci",
        disableEmbeddedWebUi: true,
        client: httpClient(new Response("<html>cybervinci</html>", { headers: { "content-type": "text/html" } })),
      }).request(`/?auth_token=${btoa("cybervinci:secret")}`)
  ...
  it.live("accepts basic auth for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("cybervinci:secret")}` },
      })
  ...
  it.live("accepts basic auth passwords containing colons for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("cybervinci:sec:ret")}` },
      })
  ...
  it.live("allows web UI preflight without auth", () =>
    Effect.gen(function* () {
      const response = yield* app({ password: "secret", username: "cybervinci" }).request("/", {
        method: "OPTIONS",
        headers: {
