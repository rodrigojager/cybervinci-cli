...
const originalWorkspaces = Flag.CYBERVINCI_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.CYBERVINCI_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session HttpApi", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        Flag.CYBERVINCI_EXPERIMENTAL_WORKSPACES = true
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const currentDir = path.join(test.directory, "packages", "cybervinci", "src")
