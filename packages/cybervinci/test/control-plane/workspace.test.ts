...
import { EventSequenceTable } from "@cybervinci-ai/core/event/sql"
...
import { WorkspaceV2 } from "@cybervinci-ai/core/workspace"
import { WorkspaceTable } from "@cybervinci-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  CYBERVINCI_AUTH_CONTENT: process.env.CYBERVINCI_AUTH_CONTENT,
  CYBERVINCI_EXPERIMENTAL_WORKSPACES: process.env.CYBERVINCI_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.CYBERVINCI_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.CYBERVINCI_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=cybervinci-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.CYBERVINCI_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.CYBERVINCI_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.CYBERVINCI_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=cybervinci-test")
