...
it.instance("plan agent denies edits except .cybervinci/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".cybervinci/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".cybervinci", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.CYBERVINCI_TEST_HOME
      process.env.CYBERVINCI_TEST_HOME = test.directory
