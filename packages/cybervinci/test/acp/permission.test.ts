...
import type { Event, CyberVinciClient } from "@cybervinci-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<CyberVinciClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "cybervinci-acp-permission-"))
