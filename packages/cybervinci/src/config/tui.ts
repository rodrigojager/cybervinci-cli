...
import { isRecord } from "@cybervinci-ai/tui/util/record"
...
import { TuiKeybind } from "@cybervinci-ai/tui/config/keybind"
...
import { TuiConfig } from "@cybervinci-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@cybervinci/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old cybervinci.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.cybervinci`
  // folders between cwd and home, and CYBERVINCI_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.CYBERVINCI_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit CYBERVINCI_TUI_CONFIG override, if set.
  if (Flag.CYBERVINCI_TUI_CONFIG) {
    const configFile = Flag.CYBERVINCI_TUI_CONFIG
  ...
  // 4. `.cybervinci` directories (and CYBERVINCI_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".cybervinci") || dir === Flag.CYBERVINCI_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".cybervinci") && dir !== Flag.CYBERVINCI_CONFIG_DIR) continue
