...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from cybervinci.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const cybervinci = await cybervinciFiles(input)
  for (const file of cybervinci) {
...
async function cybervinciFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "cybervinci"),
    ...(await Filesystem.findUp(["cybervinci.json", "cybervinci.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "cybervinci"))
  }
  if (Flag.CYBERVINCI_CONFIG) files.push(Flag.CYBERVINCI_CONFIG)
