...
import { NonNegativeInt, PositiveInt } from "@cybervinci-ai/core/schema"
import { ConfigAttachmentV1 } from "@cybervinci-ai/core/v1/config/attachment"
import { ConfigLSPV1 } from "@cybervinci-ai/core/v1/config/lsp"
import { InvalidError } from "@cybervinci-ai/core/v1/config/error"
  ...
  if (permissions.length)
    throw new InvalidError({
      ...
      issues: permissions.map((path) => ({
        path,
        message: 'V2 permissions are not supported by CYBERVINCI V1. Use V1 "permission" rules or run opencode2.',
      })),
    })
