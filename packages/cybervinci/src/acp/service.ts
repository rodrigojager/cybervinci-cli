...
import type { AssistantMessage, Message, CyberVinciClient, SessionMessageResponse } from "@cybervinci-ai/sdk/v2"
...
export const AuthMethodID = "cybervinci-login"
...
export class Service extends Context.Service<Service, Interface>()("@cybervinci/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `cybervinci auth login` in the terminal",
      name: "Login with cybervinci",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: CyberVinciClient) {
...
function makeUsageService(sdk: CyberVinciClient) {
...
async function loadDirectorySnapshot(sdk: CyberVinciClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, CYBERVINCI service provider, then sorted best model keep
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "CYBERVINCI prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "CYBERVINCI service failure", service })
}
