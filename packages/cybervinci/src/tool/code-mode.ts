...
import { CodeMode, Tool as SandboxTool, toolError } from "@cybervinci-ai/codemode"
import { MCP } from "@/mcp"
...
const invokeChildTool = Effect.fn("CodeMode.invokeChildTool")(function* (input: {
  ...
  const result: CallToolResult = yield* Effect.gen(function* () {
    ...
    return yield* Effect.promise(async () => {
      const raw = await McpDeadline.supervise({
        signal: input.ctx.abort,
        call: ({ signal, onprogress }) =>
          input.entry.tool.client.callTool(
            ...
            CallToolResultSchema,
            {
              ...
              // Code mode shares the same idle/maximum deadline contract as regular MCP tools.
              onprogress,
            },
          ),
      })
  ...
  Effect.gen(function* () {
    ...
    const init: Tool.DefWithoutID<typeof Parameters, Metadata> = {
      ...
      execute: Effect.fn("CodeMode.execute")(function* (params, ctx) {
        ...
        let childCalls = 0
        let childDeadline: McpDeadline.DeadlineError | undefined
        const callTool = (entry: CatalogEntry) => (input: unknown) =>
          Effect.gen(function* () {
            ...
            Effect.catchCause((cause) => {
              ...
              if (error instanceof McpDeadline.DeadlineError) childDeadline = error
        ...
        if (!result.ok) {
          ...
          if (childDeadline) return yield* Effect.fail(childDeadline)
