...
export type PatchDeps = {
  ...
  files: (dir: string, name: "cybervinci" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".cybervinci")
}

function patchName(kind: Kind): "cybervinci" | "tui" {
  if (kind === "server") return "cybervinci"
  return "tui"
}
