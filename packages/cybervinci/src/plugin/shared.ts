...
export async function checkPluginCompatibility(target: string, cybervinciVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(cybervinciVersion) || semver.major(cybervinciVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.cybervinci
  if (typeof range !== "string") return
  if (!semver.satisfies(cybervinciVersion, range)) {
    throw new Error(`Plugin requires cybervinci ${range} but running ${cybervinciVersion}`)
  }
}
