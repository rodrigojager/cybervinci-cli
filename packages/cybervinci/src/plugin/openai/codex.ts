import type { Hooks, PluginInput } from "@cybervinci-ai/plugin"
...
import { OauthCallbackPage } from "@cybervinci-ai/core/oauth/page"
...
function resolveCodexModelAlias(body: BodyInit | null | undefined) {
  if (typeof body !== "string") return body
  try {
    const payload: unknown = JSON.parse(body)
    if (!payload || typeof payload !== "object" || !("model" in payload) || typeof payload.model !== "string")
      return body
    const model = CODEX_MODEL_ALIASES.get(payload.model)
...
export async function CodexAuthPlugin(input: PluginInput, options: CodexAuthPluginOptions = {}): Promise<Hooks> {
  ...
  return {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            const requestInit = {
              ...init,
              headers,
              body: resolveCodexModelAlias(init?.body),
            }
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers.originator = "cybervinci"
      output.headers["User-Agent"] = `cybervinci/${InstallationVersion} (${os.platform()} ${os.release()}; ${os.arch()})`
