...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@cybervinci-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@cybervinci-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@cybervinci-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@cybervinci-ai/tui/context/theme"
import { Global } from "@cybervinci-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@cybervinci-ai/core/util/flock"
import { Flag } from "@cybervinci-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@cybervinci-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@cybervinci-ai/core/v1/config/plugin"
import { createCommandShim } from "@cybervinci-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@cybervinci-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".cybervinci"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".cybervinci", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".cybervinci", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.CYBERVINCI_PURE ? [] : pluginOrigins
    if (Flag.CYBERVINCI_PURE && pluginOrigins.length) {
    }
