      ...
      execute(args, options) {
        return run.promise(
          Effect.gen(function* () {
            ...
            Effect.tap((output) => input.processor.completeToolCall(options.toolCallId, output)),
            Effect.onExit((exit) =>
              Exit.isFailure(exit) && !Cause.hasInterrupts(exit.cause)
                ? input.processor.failToolCall(options.toolCallId, Cause.squash(exit.cause)).pipe(Effect.ignore)
                : Effect.void,
            ),
          ),
        )
      ...
      execute(args, opts) {
        return run.promise(
          Effect.gen(function* () {
            ...
            yield* input.processor.startToolCall(opts.toolCallId, MCP_RESOURCE_TOOLS.list, toRecord(args))
            ...
            Effect.tap((output) => input.processor.completeToolCall(opts.toolCallId, output)),
            Effect.onExit((exit) =>
              Exit.isFailure(exit) && !Cause.hasInterrupts(exit.cause)
                ? input.processor.failToolCall(opts.toolCallId, Cause.squash(exit.cause)).pipe(Effect.ignore)
                : Effect.void,
            ),
          ),
        )
    ...
    tools[MCP_RESOURCE_TOOLS.listTemplates] = tool({
      ...
      execute(args, opts) {
        return run.promise(
          Effect.gen(function* () {
            const ctx = context(toRecord(args), opts)
            yield* input.processor.startToolCall(opts.toolCallId, MCP_RESOURCE_TOOLS.listTemplates, toRecord(args))
            const parsed = parseListMcpResourcesArgs(args)
            const clients = yield* mcp.clients()
            const resourceServers = Object.entries(clients)
              .filter((entry) => !!entry[1].getServerCapabilities()?.resources)
    ...
    tools[MCP_RESOURCE_TOOLS.read] = tool({
      ...
      execute(args, opts) {
        return run.promise(
          Effect.gen(function* () {
            const ctx = context(toRecord(args), opts)
            yield* input.processor.startToolCall(opts.toolCallId, MCP_RESOURCE_TOOLS.read, toRecord(args))
