...
import { CYBERVINCIHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for cybervinci serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(CYBERVINCIHttpApi)
...
export const PublicApi = CYBERVINCIHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "cybervinci",
    version: "1.0.0",
    description: "cybervinci api",
