...
import { McpDeadline } from "./deadline"
...
export function convertTool(mcpTool: MCPToolDef, client: Client, timeout?: number): Tool {
  ...
  return dynamicTool({
    ...
    execute: async (args: unknown, options) => {
      const result = await McpDeadline.supervise({
        ...
        call: ({ signal, onprogress }) =>
          client.callTool(
            ...
            CallToolResultSchema,
            {
              ...
              signal,
              timeout,
              // Progress may renew the SDK's idle timeout, but never CyberVinci's maximum deadline.
              onprogress,
            },
          ),
      })
