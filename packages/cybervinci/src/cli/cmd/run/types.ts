// Shared type vocabulary for the direct interactive mode (`cybervinci --mini`).
//
...
import type { CyberVinciClient, PermissionRequest, QuestionRequest, ToolPart } from "@cybervinci-ai/sdk/v2"
import type { TuiConfig } from "@cybervinci-ai/tui/config"
...
type PromptModel = Parameters<CyberVinciClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<CyberVinciClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<CyberVinciClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<CyberVinciClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<CyberVinciClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<CyberVinciClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<CyberVinciClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<CyberVinciClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<CyberVinciClient["question"]["reject"]>[0]
