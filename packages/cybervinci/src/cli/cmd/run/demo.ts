...
import type { Event, ToolPart } from "@cybervinci-ai/sdk/v2"
...
function emitTask(state: State): void {
  ...
  const part = {
    ...
    state: {
      status: "running",
      input: {
        filePath: "packages/cybervinci/src/cli/cmd/run/stream.ts",
        offset: 1,
        limit: 200,
      },
      time: {
  ...
  if (kind === "read") {
    ...
    askPermission(state, {
      ...
      always: [target],
        ...
        title: "read",
        output: ["1: {", '2:   "name": "cybervinci",', '3:   "private": true', "4: }"].join("\n"),
        metadata: {},
      },
    })
    return
