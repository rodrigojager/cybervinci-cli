...
import type { Hooks } from "@cybervinci-ai/plugin"
...
const handlePluginAuth = Effect.fn("Cli.providers.pluginAuth")(function* (
  ...
  if (method.type === "oauth") {
    ...
    if (authorize.url) {
      ...
      yield* openBrowser(authorize.url)
    }
...
export const ProvidersLoginCommand = effectCmd({
  ...
  builder: (yargs: Argv) =>
    yargs
      .positional("url", {
        describe: "cybervinci auth provider",
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    if (args.url) {
      ...
      const wellknown = (yield* cliTry(`Failed to load auth provider metadata from ${url}: `, () =>
        fetch(`${url}/.well-known/cybervinci`).then((x) => x.json()),
      )) as {
    ...
    if (provider === "other") {
      ...
      yield* Prompt.log.warn(
        `This only stores a credential for ${provider} - you will need configure it in cybervinci.json, check the docs for examples.`,
      )
    }
    ...
    if (provider === "amazon-bedrock") {
      yield* Prompt.log.info(
        "Amazon Bedrock authentication priority:\n" +
          ...
          "Configure via cybervinci.json options (profile, region, endpoint) or\n" +
