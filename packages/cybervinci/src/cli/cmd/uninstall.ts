...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall cybervinci and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall CYBERVINCI")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g cybervinci-ai",
      pnpm: "pnpm uninstall -g cybervinci-ai",
      bun: "bun remove -g cybervinci-ai",
      yarn: "yarn global remove cybervinci-ai",
      brew: "brew uninstall cybervinci",
      choco: "choco uninstall cybervinci",
      scoop: "scoop uninstall cybervinci",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "cybervinci", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using CYBERVINCI!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# cybervinci") || content.includes(".cybervinci/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".cybervinci/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".cybervinci/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".cybervinci"))
    ) {
      continue
    }
