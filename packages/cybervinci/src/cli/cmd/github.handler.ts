...
const WORKFLOW_FILE = ".github/workflows/cybervinci.yml"
...
export const githubInstall = Effect.fn("Cli.github.install")(function* () {
  ...
  yield* Effect.promise(async () => {
    {
      UI.empty()
      prompts.intro("Install CYBERVINCI GitHub agent")
      const actionRef = process.env["CYBERVINCI_GITHUB_ACTION"]?.trim()
      const actionBinary = process.env["CYBERVINCI_GITHUB_BINARY"]?.trim()
      if (!actionRef || !actionBinary) {
        prompts.log.error(
          "Self-hosted GitHub integration requires CYBERVINCI_GITHUB_ACTION and CYBERVINCI_GITHUB_BINARY.",
        )
        throw new UI.CancelledError()
      }
      if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+(?:\/[A-Za-z0-9_./-]+)?@[A-Za-z0-9_./-]+$/.test(actionRef)) {
        prompts.log.error("CYBERVINCI_GITHUB_ACTION must be a pinned owner/repository[/path]@ref action reference.")
        throw new UI.CancelledError()
      }
      ...
      function printNextSteps() {
        ...
        prompts.outro(
          [
            ...
            step2,
            "",
            "    3. Go to a GitHub issue and comment `/cybervinci summarize` to see the agent in action",
            "",
            "   This workflow uses the explicitly configured self-hosted CYBERVINCI Action and binary.",
          ].join("\n"),
        )
      }
...
jobs:
  cybervinci:
    if: |
      ...
      contains(github.event.comment.body, ' /cybervinci') ||
      startsWith(github.event.comment.body, '/cybervinci')
    ...
    steps:
      ...
      - name: Run cybervinci
        uses: ${actionRef}${envStr}
        with:
          binary: ${JSON.stringify(actionBinary)}
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    let gitConfig: string | null | undefined
    ...
    const agentUsername = useGithubToken ? "github-actions[bot]" : AGENT_USERNAME
    ...
    try {
      if (useGithubToken) {
        ...
        appToken = await exchangeForAppToken(actionToken, normalizeOidcBaseUrl())
      }
      ...
      console.log("cybervinci session", session.id)
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/cybervinci,/oc")
        .split(",")
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log("Sending message to cybervinci...")
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("cybervinci-github-action")
      } catch (error) {
    ...
    async function exchangeForAppToken(token: string, oidcBaseUrl: string) {
    ...
    async function configureGit(appToken: string) {
      ...
      gitConfig = ret.exitCode === 0 ? ret.stdout.toString().trim() : null
      if (ret.exitCode === 0) await gitRun(["config", "--local", "--unset-all", config])
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `cybervinci/${type}-${hex}-${timestamp}`
      }
      return `cybervinci/${type}${issueId}-${timestamp}`
    }
    ...
    function footer(opts?: { image?: boolean }) {
      ...
      const shareUrl = shareId ? `[cybervinci session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the cybervinci infrastructure after your response",
