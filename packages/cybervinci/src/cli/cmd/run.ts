import type { PermissionV1 } from "@cybervinci-ai/core/v1/permission"
import { FSUtil } from "@cybervinci-ai/core/fs-util"
// CLI entry point for `cybervinci run` and `cybervinci --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`cybervinci --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`cybervinci --mini --attach`): connects to a running
//      cybervinci server and runs interactive mode against it.
//
...
import { createCyberVinciClient, type CyberVinciClient, type ToolPart } from "@cybervinci-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<CyberVinciClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run cybervinci with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running cybervinci server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to CYBERVINCI_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to CYBERVINCI_SERVER_USERNAME or 'cybervinci')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: CyberVinciClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: CyberVinciClient, sessionID: string) {
      ...
      async function current(sdk: CyberVinciClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: CyberVinciClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: CyberVinciClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: CyberVinciClient) {
        ...
        async function loop(client: CyberVinciClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createCyberVinciClient({
        baseUrl: "http://cybervinci.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
