...
import { createCyberVinciClient } from "@cybervinci-ai/sdk/v2/client"
import { ServerAuth } from "@cybervinci-ai/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createCyberVinciClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@cybervinci/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createCyberVinciClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createCyberVinciClient({ baseUrl: connection.url, headers: connection.headers })
    })
