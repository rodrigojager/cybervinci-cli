---
title: Configuration
description: Utilisation de la configuration JSON CYBERVINCI.
---

Vous pouvez configurer CYBERVINCI à l'aide d'un fichier de configuration JSON.

---

## Format

CYBERVINCI prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
1. **Configuration distante** (à partir de `.well-known/cybervinci`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/cybervinci/cybervinci.json`) - préférences utilisateur
3. **Configuration personnalisée** (`CYBERVINCI_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`cybervinci.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.cybervinci`** - agents, commandes, plugins
6. **Configuration en ligne** (`CYBERVINCI_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.cybervinci` et `~/.config/cybervinci` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/cybervinci`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale CYBERVINCI dans `~/.config/cybervinci/cybervinci.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/cybervinci/tui.json`.
...
Ajoutez `cybervinci.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque CYBERVINCI démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `CYBERVINCI_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `CYBERVINCI_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.cybervinci`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.cybervinci`, il **peut donc remplacer** leurs paramètres.
...
Utilisez `CYBERVINCI_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `cybervinci.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `cybervinci serve` et `cybervinci web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur CYBERVINCI.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `cybervinci.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration CYBERVINCI via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, CYBERVINCI essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/cybervinci/agents/` ou `.cybervinci/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, CYBERVINCI reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`cybervinci run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/cybervinci/commands/` ou `.cybervinci/commands/`. [En savoir plus ici](/docs/commands).
...
CYBERVINCI téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, cybervinci **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent CYBERVINCI avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.cybervinci/plugins/` ou `~/.config/cybervinci/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
...
Ceci est utile lorsque vous souhaitez restreindre CYBERVINCI à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
