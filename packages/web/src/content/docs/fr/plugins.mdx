---
title: Plugins
description: Écrivez vos propres plugins pour étendre CYBERVINCI.
---

Les plugins vous permettent d'étendre CYBERVINCI en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de CYBERVINCI.
...
- `.cybervinci/plugins/` - Plugins au niveau du projet
- `~/.config/cybervinci/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/cybervinci/node_modules/`.
...
1. Configuration globale (`~/.config/cybervinci/cybervinci.json`)
2. Configuration du projet (`cybervinci.json`)
3. Répertoire global des plugins (`~/.config/cybervinci/plugins/`)
4. Répertoire des plugins du projet (`.cybervinci/plugins/`)
...
CYBERVINCI exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK cybervinci pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre cybervinci.
...
:::note
Si vous utilisez l'application de bureau CYBERVINCI, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez cybervinci de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à cybervinci :
...
L'assistant `tool` crée un outil personnalisé que cybervinci peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour cybervinci aux côtés des outils intégrés.
