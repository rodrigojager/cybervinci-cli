...
Avec CYBERVINCI, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, CYBERVINCI utilise notre propre thème `cybervinci`.
...
CYBERVINCI est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que CYBERVINCI corresponde à l'apparence de leur terminal
...
Select a theme with the /theme command, or set it in tui.json.
...
CYBERVINCI prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/cybervinci/themes/*.json` ou `$XDG_CONFIG_HOME/cybervinci/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.cybervinci/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.cybervinci/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/cybervinci/themes
vim ~/.config/cybervinci/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .cybervinci/themes
vim .cybervinci/themes/my-theme.json
```
