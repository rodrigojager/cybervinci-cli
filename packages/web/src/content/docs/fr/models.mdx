...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez CYBERVINCI.
...
Voici plusieurs modèles qui fonctionnent bien avec CYBERVINCI, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration CYBERVINCI.

```json title="cybervinci.json" {3}
{
...
```jsonc title="cybervinci.jsonc" {7-12,19-24}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="cybervinci.jsonc" {6-21}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "opencode": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. CYBERVINCI est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

CYBERVINCI est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="cybervinci.jsonc" {7-18}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque CYBERVINCI démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration CYBERVINCI.
