---
title: Config
description: Using the CYBERVINCI JSON config.
---

You can configure CYBERVINCI using a JSON config file.

---

## Format

CYBERVINCI supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="cybervinci.jsonc"
{
...
1. **Remote config** (from `.well-known/cybervinci`) - organizational defaults
2. **Global config** (`~/.config/cybervinci/cybervinci.json`) - user preferences
3. **Custom config** (`CYBERVINCI_CONFIG` env var) - custom overrides
4. **Project config** (`cybervinci.json` in project) - project-specific settings
5. **`.cybervinci` directories** - agents, commands, plugins
6. **Inline config** (`CYBERVINCI_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/cybervinci/` on macOS) - admin-controlled
...
:::note
The `.cybervinci` and `~/.config/cybervinci` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/cybervinci` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/cybervinci"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global CYBERVINCI config in `~/.config/cybervinci/cybervinci.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/cybervinci/tui.json`.
...
Add `cybervinci.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When CYBERVINCI starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `CYBERVINCI_CONFIG` environment variable.

```bash
export CYBERVINCI_CONFIG=/path/to/my/custom-config.json
cybervinci run "Hello world"
```
...
Specify a custom config directory using the `CYBERVINCI_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.cybervinci` directory, and should
follow the same structure.

```bash
export CYBERVINCI_CONFIG_DIR=/path/to/my/config-directory
cybervinci run "Hello world"
```

The custom directory is loaded after the global config and `.cybervinci` directories, so it **can override** their settings.
...
Drop an `cybervinci.json` or `cybervinci.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/cybervinci/` |
| Linux    | `/etc/cybervinci/`                         |
| Windows  | `%ProgramData%\cybervinci`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, CYBERVINCI reads managed preferences from the `ai.cybervinci.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

CYBERVINCI checks these paths:

1. `/Library/Managed Preferences/<user>/ai.cybervinci.managed.plist`
2. `/Library/Managed Preferences/ai.cybervinci.managed.plist`

The plist keys map directly to `cybervinci.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.cybervinci.managed` PayloadType. The CYBERVINCI config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.cybervinci.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.cybervinci.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.cybervinci</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
Use `CYBERVINCI_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `cybervinci.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `cybervinci serve` and `cybervinci web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your CYBERVINCI server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `cybervinci.local`. Useful for running multiple instances on the same network.
...
If not specified, CYBERVINCI will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your CYBERVINCI config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, CYBERVINCI tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny CYBERVINCI actions on configured resources. Currently, policies can control which providers CYBERVINCI may use.
...
CYBERVINCI normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, CYBERVINCI omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
Set the TUI theme in tui.json with the theme option.
...
You can also define agents using markdown files in `~/.config/cybervinci/agents/` or `.cybervinci/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, CYBERVINCI will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`cybervinci run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/cybervinci/commands/` or `.cybervinci/commands/`. [Learn more here](/docs/commands).
...
CYBERVINCI uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
CYBERVINCI will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, cybervinci **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend CYBERVINCI with custom tools, hooks, and integrations.

Place plugin files in `.cybervinci/plugins/` or `~/.config/cybervinci/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict CYBERVINCI to only use specific providers rather than disabling them one by one.
