---
title: Server
description: Interact with cybervinci server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `cybervinci serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an cybervinci client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `cybervinci.local` |
...
Set `CYBERVINCI_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `cybervinci`, or set `CYBERVINCI_SERVER_USERNAME` to override it. This applies to both `cybervinci serve` and `cybervinci web`.
...
When you run `cybervinci` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the cybervinci server to interact with cybervinci programmatically.
:::

This architecture lets cybervinci support multiple clients and allows you to interact with cybervinci programmatically.

You can run `cybervinci serve` to start a standalone server. If you have the
cybervinci TUI running, `cybervinci serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the CYBERVINCI [IDE](/docs/ide) plugins.
...
The cybervinci server exposes the following APIs.
