---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع CYBERVINCI.
---

تتيح لك الإضافات توسيع CYBERVINCI عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ CYBERVINCI.
...
- `.cybervinci/plugins/` - إضافات على مستوى المشروع
- `~/.config/cybervinci/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/cybervinci/node_modules/`.
...
1. إعدادات عامة (`~/.config/cybervinci/cybervinci.json`)
2. إعدادات المشروع (`cybervinci.json`)
3. دليل الإضافات العام (`~/.config/cybervinci/plugins/`)
4. دليل إضافات المشروع (`.cybervinci/plugins/`)
...
يشغّل CYBERVINCI الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ cybervinci للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع cybervinci.
...
:::note
إذا كنت تستخدم تطبيق CYBERVINCI لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع cybervinci من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى cybervinci:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ cybervinci استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ cybervinci إلى جانب الأدوات المضمنة.
