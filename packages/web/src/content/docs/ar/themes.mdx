...
مع CYBERVINCI يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم CYBERVINCI سمتنا `cybervinci`.
...
يأتي CYBERVINCI مع عدة سمات مدمجة.
...
- يريدون أن يطابق CYBERVINCI مظهر terminal لديهم
...
يدعم CYBERVINCI نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/cybervinci/themes/*.json` أو `$XDG_CONFIG_HOME/cybervinci/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.cybervinci/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.cybervinci/themes/*.json`
