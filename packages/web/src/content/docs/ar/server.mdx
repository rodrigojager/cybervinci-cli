---
title: الخادم
description: تفاعل مع خادم cybervinci عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `cybervinci serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل cybervinci استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `cybervinci.local` |
...
عيّن `CYBERVINCI_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `cybervinci`، أو عيّن `CYBERVINCI_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `cybervinci serve` و `cybervinci web`.
...
عند تشغيل `cybervinci` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم cybervinci للتفاعل مع cybervinci برمجيا.
:::

تتيح هذه البنية لـ cybervinci دعم عدة عملاء وتمكّنك من التفاعل مع cybervinci برمجيا.

يمكنك تشغيل `cybervinci serve` لبدء خادم مستقل. إذا كانت واجهة cybervinci في terminal (TUI)
قيد التشغيل، فسيبدأ `cybervinci serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات CYBERVINCI لـ [IDE](/docs/ide).
...
يعرض خادم cybervinci واجهات API التالية.
