...
يوجد نوعان من الوكلاء في CYBERVINCI: وكلاء أساسيون ووكلاء فرعيون.
...
يأتي CYBERVINCI مع وكيلين أساسيين مدمجين: **Build** و **Plan**. سنلقي نظرة عليهما أدناه.
...
يأتي CYBERVINCI مع ثلاثة وكلاء فرعيين مدمجين: **General** و **Explore** و **Scout**. سنلقي نظرة على ذلك أدناه.

---

## المدمجة

يأتي CYBERVINCI مع وكيلين أساسيين مدمجين وثلاثة وكلاء فرعيين مدمجين.
...
وكيل للقراءة فقط مخصص للوثائق الخارجية وأبحاث التبعيات. استخدمه عندما تحتاج إلى استنساخ مستودع تبعية داخل ذاكرة التخزين المؤقت المُدارة في CYBERVINCI، أو فحص الشفرة المصدرية لمكتبة، أو إجراء مراجع متقاطعة بين الشفرة المحلية والتنفيذات upstream بدون تعديل مساحة العمل الخاصة بك.
...
هيّئ الوكلاء في ملف إعدادات `cybervinci.json`:
...
- عام: `~/.config/cybervinci/agents/`
- لكل مشروع: `.cybervinci/agents/`
...
إذا لم يتم تحديد temperature، يستخدم CYBERVINCI القيم الافتراضية الخاصة بالنموذج؛ عادة 0 لمعظم النماذج و 0.55 لنماذج Qwen.
...
هذا المسار نسبي لمكان وجود ملف الإعدادات. لذلك يعمل هذا لكل من إعدادات CYBERVINCI العامة وإعدادات المشروع.
...
يستخدم معرّف النموذج في إعدادات CYBERVINCI الصيغة `provider/model-id`. على سبيل المثال، إذا كنت تستخدم [OpenCode Zen](/docs/zen)، فستستخدم `opencode/gpt-5.1-codex` لـ GPT 5.1 Codex.
...
:::tip
شغّل `cybervinci models` لعرض قائمة بالنماذج المتاحة.
:::
