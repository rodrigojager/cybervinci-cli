...
تتيح مهارات الوكيل لـ CYBERVINCI اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث CYBERVINCI في هذه المواقع:

- إعدادات المشروع: `.cybervinci/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/cybervinci/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج CYBERVINCI صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.cybervinci/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/cybervinci/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.cybervinci/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض CYBERVINCI المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `cybervinci.json`:
...
**للوكلاء المدمجين** (في `cybervinci.json`):
