...
أنشئ الملف `.cybervinci/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات CYBERVINCI أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ CYBERVINCI:
...
- على مستوى النظام: `~/.config/cybervinci/commands/`
- لكل مشروع: `.cybervinci/commands/`
...
يتضمن cybervinci عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
