---
title: ACP Desteği
description: cybervinci'u ACP uyumlu herhangi bir editörde kullanın.
---

cybervinci, [Agent Client Protocol](https://agentclientprotocol.com) veya (ACP) dosyalarını destekleyerek onu doğrudan uyumlu editörlerde ve IDE'lerde kullanmanızı sağlar.
...
cybervinci'u ACP aracılığıyla kullanmak için düzenleyicinizi `cybervinci acp` komutunu çalıştıracak şekilde yapılandırın.

Komut, cybervinci'u, editörünüzle stdio aracılığıyla JSON-RPC üzerinden iletişim kuran ACP uyumlu bir alt süreç olarak başlatır.
...
Açmak için AI Chat agent seçicisindeki yeni 'CYBERVINCI' ajanını kullanın.
...
cybervinci'u [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)'de ACP ajanı olarak kullanmak için Neovim yapılandırmanıza aşağıdakileri ekleyin:
...
Bu yapılandırma, CodeCompanion'ı cybervinci'u sohbet için ACP ajanı olarak kullanacak şekilde ayarlar.
...
cybervinci, terminalde olduğu gibi ACP aracılığıyla aynı şekilde çalışır. Tüm özellikler desteklenir:
...
- cybervinci yapılandırmanızda yapılandırılmış MCP sunucuları
