...
cybervinci ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak cybervinci kendi `cybervinci` temasını kullanır.
...
cybervinci birden fazla yerleşik temayla gelir.
...
- cybervinci'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
cybervinci, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/cybervinci/themes/*.json` veya `$XDG_CONFIG_HOME/cybervinci/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.cybervinci/themes/*.json`
4. **Geçerli çalışma dizini** - `./.cybervinci/themes/*.json`
