---
title: Eklentiler
description: CYBERVINCI'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek CYBERVINCI'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya CYBERVINCI'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.cybervinci/plugins/` - Proje düzeyinde eklentiler
- `~/.config/cybervinci/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/cybervinci/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/cybervinci/cybervinci.json`)
2. Project config (`cybervinci.json`)
3. Global eklenti dizini (`~/.config/cybervinci/plugins/`)
4. Proje eklenti dizini (`.cybervinci/plugins/`)
...
CYBERVINCI bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir CYBERVINCI SDK istemcisi.
...
CYBERVINCI'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
CYBERVINCI masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
CYBERVINCI'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca CYBERVINCI'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, CYBERVINCI'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra CYBERVINCI'a da hazır olacaktır.
