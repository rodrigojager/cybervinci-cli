---
title: TUI
description: CYBERVINCI terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CYBERVINCI, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

CYBERVINCI'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
CYBERVINCI TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
CYBERVINCI'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
CYBERVINCI'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `cybervinci.json` dosyasından ayrıdır.
...
- `leader_timeout` - CYBERVINCI'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
...
- `sound_pack` - Kullanılacak sound pack ID. Varsayılan değer `cybervinci.default`.
