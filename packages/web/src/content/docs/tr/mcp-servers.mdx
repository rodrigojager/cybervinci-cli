...
_Model Bağlam Protokolü_ veya MCP kullanarak cybervinci'a harici araçlar ekleyebilirsiniz. cybervinci hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [cybervinci Config](/docs/config)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/cybervinci` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
cybervinci, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde cybervinci şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa cybervinci, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `cybervinci mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından cybervinci, belirteçleri `~/.local/share/cybervinci/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra cybervinci'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi cybervinci yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve cybervinci'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
