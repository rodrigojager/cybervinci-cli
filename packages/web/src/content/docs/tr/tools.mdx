...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. cybervinci yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
cybervinci içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `CYBERVINCI_EXPERIMENTAL_LSP_TOOL=true` (veya `CYBERVINCI_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca OpenCode provider kullanılırken veya `CYBERVINCI_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

cybervinci başlatırken etkinleştirmek için:
