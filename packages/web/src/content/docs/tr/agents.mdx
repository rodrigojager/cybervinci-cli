...
cybervinci'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
cybervinci, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
CYBERVINCI, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

CYBERVINCI iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `cybervinci.json` yapılandırma dosyanızda yapılandırın:
...
- Proje başına: `.cybervinci/agents/`
...
Sıcaklık belirtilmezse cybervinci modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global cybervinci yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
cybervinci yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [OpenCode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `opencode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `cybervinci models` komutunu çalıştırın.
:::
