---
title: Yapılandırma
description: cybervinci JSON yapılandırması kullanır.
---

cybervinci'u JSON yapılandırma dosyası kullanarak yapılandırabilirsiniz.

---

## Format

cybervinci hem **JSON** hem de **JSONC** (JSON Yorumlarla birlikte) formatlarını destekler.
...
1. **Uzak yapılandırma** (`.well-known/cybervinci`'dan) - kurumsal varsayılanlar
2. **Genel yapılandırma** (`~/.config/cybervinci/cybervinci.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`CYBERVINCI_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `cybervinci.json`) - projeye özgü ayarlar
5. **`.cybervinci` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`CYBERVINCI_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
:::note
`.cybervinci` ve `~/.config/cybervinci` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
Kuruluşlar, `.well-known/cybervinci` uç noktası aracılığıyla varsayılan yapılandırmayı sağlayabilir. Bu, onu destekleyen bir sağlayıcıyla kimlik doğrulaması yaptığınızda otomatik olarak alınır.
...
Global cybervinci yapılandırmanızı `~/.config/cybervinci/cybervinci.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `cybervinci.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.

:::tip
Projeye özel yapılandırmayı projenizin köküne yerleştirin.
:::

cybervinci başlatıldığında geçerli dizinde bir yapılandırma dosyası arar veya en yakın Git dizinine gider.
...
`CYBERVINCI_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`CYBERVINCI_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
ortam değişkeni. Bu dizin agent'lar, komutlar için aranacaktır.
modlar ve eklentiler tıpkı standart `.cybervinci` dizini gibi olmalıdır ve
...
Özel dizin, genel yapılandırma ve `.cybervinci` dizinlerinden sonra yüklenir, böylece bunların ayarlarını **geçersiz kılabilir**.
...
`cybervinci serve` ve `cybervinci web` komutları için sunucu ayarlarını `server` seçeneği aracılığıyla yapılandırabilirsiniz.
...
- `mdns` - mDNS hizmet bulmayı etkinleştirin. Bu, ağdaki diğer cihazların cybervinci sunucunuzu keşfetmesine olanak tanır.
- `mdnsDomain` - mDNS hizmeti için özel alan adı. Varsayılan olarak `cybervinci.local` şeklindedir. Aynı ağda birden fazla örneği çalıştırmak için kullanışlıdır.
...
cybervinci yapılandırmanızda kullanmak istediğiniz sağlayıcıları ve modelleri `provider`, `model` ve `small_model` seçenekleri aracılığıyla yapılandırabilirsiniz.
...
`small_model` seçeneği, başlık oluşturma gibi hafif görevler için ayrı bir model yapılandırır. Varsayılan olarak cybervinci, sağlayıcınızda mevcutsa daha ucuz bir model kullanmaya çalışır, aksi takdirde ana modelinize geri döner.
...
Agent'ları `~/.config/cybervinci/agents/` veya `.cybervinci/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Varsayılan agent, birincil agent olmalıdır (alt agent değil). Bu, `"build"` veya `"plan"` gibi yerleşik bir agent ya da tanımladığınız bir [özel agent](/docs/agents) olabilir. Belirtilen agent mevcut değilse veya bir alt agent ise, cybervinci bir uyarıyla birlikte `"build"`'ye geri döner.

Bu ayar tüm arayüzler için geçerlidir: TUI, CLI (`cybervinci run`), masaüstü uygulaması ve GitHub Action.
...
Komutları `~/.config/cybervinci/commands/` veya `.cybervinci/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
cybervinci başlatıldığında yeni güncellemeleri otomatik olarak indirecektir. Bunu `autoupdate` seçeneğiyle devre dışı bırakabilirsiniz.
...
Varsayılan olarak, cybervinci açık bir onay gerektirmeden **tüm işlemlere izin verir**. `permission` seçeneğini kullanarak bunu değiştirebilirsiniz.
...
[Eklentiler](/docs/plugins) cybervinci'u özel araçlar, kancalar ve entegrasyonlarla genişletin.

Eklenti dosyalarını `.cybervinci/plugins/` veya `~/.config/cybervinci/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
...
Bu, cybervinci'u tek tek devre dışı bırakmak yerine yalnızca belirli sağlayıcıları kullanacak şekilde kısıtlamak istediğinizde kullanışlıdır.
