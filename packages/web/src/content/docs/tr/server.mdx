---
title: Sunucu
description: cybervinci sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`cybervinci serve` komutu, cybervinci istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `cybervinci.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `CYBERVINCI_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `cybervinci` değeridir; değiştirmek isterseniz `CYBERVINCI_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `cybervinci serve` hem de `cybervinci web` için geçerlidir.
...
`cybervinci` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
cybervinci ile programatik etkileşim için cybervinci sunucusunu kullanın.
:::

Bu mimari, cybervinci'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`cybervinci serve` ile bağımsız bir sunucu başlatabilirsiniz. cybervinci TUI açıksa, `cybervinci serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum cybervinci [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
cybervinci sunucusu aşağıdaki API'leri sunar.
