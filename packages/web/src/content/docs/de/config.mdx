---
title: Konfiguration
description: Verwenden Sie die Konfiguration CYBERVINCI JSON.
---

Sie können CYBERVINCI mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

CYBERVINCI unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
1. **Remote-Konfiguration** (von `.well-known/cybervinci`) – Organisationsstandards
2. **Globale Konfiguration** (`~/.config/cybervinci/cybervinci.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`CYBERVINCI_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`cybervinci.json` im Projekt) – projektspezifische Einstellungen
5. **`.cybervinci` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`CYBERVINCI_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
:::note
Die Verzeichnisse `.cybervinci` und `~/.config/cybervinci` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Organisationen können die Standardkonfiguration über den Endpunkt `.well-known/cybervinci` bereitstellen. Dieser wird automatisch abgerufen, wenn Sie sich bei einem Anbieter authentifizieren, der dies unterstützt.
...
Platzieren Sie Ihre globale CYBERVINCI-Konfiguration in `~/.config/cybervinci/cybervinci.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `cybervinci.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.

:::tip
Platzieren Sie die projektspezifische Konfiguration im Stammverzeichnis Ihres Projekts.
:::

Wenn CYBERVINCI gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
Geben Sie mithilfe der Umgebungsvariablen `CYBERVINCI_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `CYBERVINCI_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
Umgebungsvariable. Dieses Verzeichnis wird nach Agenten, Befehlen,
Modi und Plugins genau wie das Standardverzeichnis `.cybervinci` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.cybervinci` geladen, sodass es deren Einstellungen **überschreiben** kann.
...
Sie können Servereinstellungen für die Befehle `cybervinci serve` und `cybervinci web` über die Option `server` konfigurieren.
...
- `mdns` – mDNS-Diensterkennung aktivieren. Dadurch können andere Geräte im Netzwerk Ihres CYBERVINCI-Servers erkannt werden.
- `mdnsDomain` – Benutzerdefinierter Domänenname für den mDNS-Dienst. Standardmäßig ist `cybervinci.local`. Nützlich für die Ausführung mehrerer Instanzen im selben Netzwerk.
...
Sie können die Anbieter und Modelle, die Sie in Ihrer CYBERVINCI-Konfiguration verwenden möchten, über die Optionen `provider`, `model` und `small_model` konfigurieren.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht CYBERVINCI, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/cybervinci/agents/` oder `.cybervinci/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift CYBERVINCI mit einer Warnung auf `"build"` zurück.

Diese Einstellung gilt für alle Schnittstellen: TUI, CLI (`cybervinci run`), Desktop-App und GitHub Aktion.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/cybervinci/commands/` oder `.cybervinci/commands/` definieren. [Mehr erfahren](/docs/commands).
...
CYBERVINCI lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
CYBERVINCI erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert CYBERVINCI mit benutzerdefinierten Tools, Hooks und Integrationen.

Platzieren Sie Ihre Plugin-Dateien in `.cybervinci/plugins/` oder `~/.config/cybervinci/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
...
Dies ist nützlich, wenn Sie CYBERVINCI darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.
