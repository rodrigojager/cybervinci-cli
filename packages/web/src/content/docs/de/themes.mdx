...
Mit CYBERVINCI kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet CYBERVINCI das `cybervinci`-Theme.
...
CYBERVINCI liefert mehrere eingebaute Themes mit.
...
- CYBERVINCI optisch ans Terminal anpassen wollen
...
CYBERVINCI unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/cybervinci/themes/*.json` oder `$XDG_CONFIG_HOME/cybervinci/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.cybervinci/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.cybervinci/themes/*.json`
