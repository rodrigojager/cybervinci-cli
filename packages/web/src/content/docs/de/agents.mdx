...
Es gibt zwei Arten von Agenten in CYBERVINCI; Primäragenten und Subagenten.
...
CYBERVINCI verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `cybervinci.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet CYBERVINCI modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale CYBERVINCI-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer CYBERVINCI-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [OpenCode Zen](/docs/zen) verwenden, würden Sie `opencode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `cybervinci models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
