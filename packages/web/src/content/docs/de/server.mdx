---
title: Server
description: Nutze den cybervinci-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `cybervinci serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein cybervinci-Client nutzen kann.
...
Setze `CYBERVINCI_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `cybervinci`, kann aber mit `CYBERVINCI_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `cybervinci serve` und `cybervinci web`.
...
Wenn du `cybervinci` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den cybervinci-Server, um cybervinci programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `cybervinci serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `cybervinci serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die CYBERVINCI-[IDE](/docs/ide)-Plugins.
...
Der cybervinci-Server stellt folgende APIs bereit.
