---
title: LSP Server
description: CYBERVINCI lässt sich in Ihren LSP-Server integrieren.
---

CYBERVINCI kann sich in Language Server Protocol (LSP)-Server integrieren, um Diagnosen als Feedback für den Agenten zu verwenden.

---

## Integriert

CYBERVINCI verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `CYBERVINCI_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn LSP aktiviert ist und CYBERVINCI eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer CYBERVINCI-Konfiguration aktivieren und anpassen.
