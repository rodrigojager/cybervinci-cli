---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um CYBERVINCI zu erweitern.
---

Mit Plugins können Sie CYBERVINCI erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von CYBERVINCI zu ändern.
...
- `.cybervinci/plugins/` – Plugins auf Projektebene
- `~/.config/cybervinci/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/cybervinci/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/cybervinci/cybervinci.json`)
2. Projektkonfiguration (`cybervinci.json`)
3. Globales Plugin-Verzeichnis (`~/.config/cybervinci/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.cybervinci/plugins/`)
...
CYBERVINCI führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein CYBERVINCI-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie CYBERVINCI erweitern können.
...
:::note
Wenn Sie die CYBERVINCI-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass CYBERVINCI `.env`-Dateien liest:
...
Plugins können CYBERVINCI auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das CYBERVINCI aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für CYBERVINCI zur Verfügung.
