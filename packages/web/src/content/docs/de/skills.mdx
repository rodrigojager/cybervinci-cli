...
Agent Skills erlauben CYBERVINCI, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
CYBERVINCI sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft CYBERVINCI vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.cybervinci/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/cybervinci/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.cybervinci/skills/git-release/SKILL.md` zum Beispiel so:
...
CYBERVINCI listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `cybervinci.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `cybervinci.json`):
