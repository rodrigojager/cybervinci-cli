---
title: Regeln
description: Lege eigene Anweisungen fuer cybervinci fest.
---

Du kannst cybervinci ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in cybervinci den Befehl `/init` aus.
...
So kann cybervinci sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht cybervinci sie zu erweitern.
...
cybervinci kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/cybervinci/AGENTS.md` speichern.
Diese gelten in allen cybervinci-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt CYBERVINCI dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/cybervinci/AGENTS.md` existiert)
...
Beim Start sucht cybervinci in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/cybervinci/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `cybervinci.json` oder in `~/.config/cybervinci/cybervinci.json` angeben.
...
cybervinci parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit cybervinci.json

Empfohlen ist das `instructions`-Feld in der `cybervinci.json`:
...
Du kannst cybervinci explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `cybervinci.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
