---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in CYBERVINCI.
---

import config from "../../../../config.mjs"
export const console = config.console

CYBERVINCI verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer CYBERVINCI-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem CYBERVINCI anpassen
config.
...
OpenCode Zen ist eine Liste von Modellen, die vom CYBERVINCI-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit CYBERVINCI funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit OpenCode Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie CYBERVINCI aus und gehen Sie zu [opencode.ai/auth](https://opencode.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in CYBERVINCI und ist völlig optional.

---

## OpenCode Go

OpenCode Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom CYBERVINCI-Team getestet und verifiziert wurden, dass sie gut mit CYBERVINCI funktionieren.
...
So verwenden Sie Amazon Bedrock mit CYBERVINCI:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie CYBERVINCI ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `cybervinci.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in CYBERVINCI wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können cybervinci so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit CYBERVINCI ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre CYBERVINCI-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `CYBERVINCI`, Bereiche: `api`
...
um das Token im CYBERVINCI-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
CYBERVINCI verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von CYBERVINCI
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`cybervinci.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit CYBERVINCI:
...
So verwenden Sie Google Vertex AI mit CYBERVINCI:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie CYBERVINCI ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über CYBERVINCI konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session)-Plugin, um jede CYBERVINCI-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede CYBERVINCI-Konversation als separate Sitzung aufgeführt.
...
Sie können CYBERVINCI für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können CYBERVINCI über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können CYBERVINCI über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für CYBERVINCI konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/opencode).
:::
...
So verwenden Sie Ollama Cloud mit CYBERVINCI:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in CYBERVINCI.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in CYBERVINCI verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
OpenCode Zen ist eine Liste getesteter und verifizierter Modelle, die vom CYBERVINCI-Team bereitgestellt werden. [Learn more](/docs/zen).
...
3. Geben Sie Ihren CYBERVINCI API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre CYBERVINCI-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre CYBERVINCI-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit CYBERVINCI:
...
Sie können Modelle auch über Ihre CYBERVINCI-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit CYBERVINCI verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `cybervinci.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann CYBERVINCI erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `cybervinci auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die CYBERVINCI-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer CYBERVINCI-Konfiguration übereinstimmt.
