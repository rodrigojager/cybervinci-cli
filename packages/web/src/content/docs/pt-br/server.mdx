---
title: Servidor
description: Interaja com o servidor cybervinci via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `cybervinci serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente cybervinci pode usar.
...
```bash
cybervinci serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `cybervinci.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
cybervinci serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `CYBERVINCI_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `cybervinci`, ou defina `CYBERVINCI_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `cybervinci serve` quanto ao `cybervinci web`.

```bash
CYBERVINCI_SERVER_PASSWORD=your-password cybervinci serve
```
...
Quando você executa `cybervinci`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor cybervinci para interagir com o cybervinci programaticamente.
:::

Essa arquitetura permite que o cybervinci suporte múltiplos clientes e permite que você interaja com o cybervinci programaticamente.

Você pode executar `cybervinci serve` para iniciar um servidor autônomo. Se você tiver o TUI do cybervinci em execução, `cybervinci serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do cybervinci [IDE](/docs/ide).
...
O servidor cybervinci expõe as seguintes APIs.
