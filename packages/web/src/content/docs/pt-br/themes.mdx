...
Com o cybervinci, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o cybervinci usa nosso próprio tema `cybervinci`.
...
O cybervinci vem com vários temas embutidos.
...
- Querem que o cybervinci corresponda à aparência do seu terminal
...
O cybervinci suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/cybervinci/themes/*.json` ou `$XDG_CONFIG_HOME/cybervinci/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.cybervinci/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.cybervinci/themes/*.json`
