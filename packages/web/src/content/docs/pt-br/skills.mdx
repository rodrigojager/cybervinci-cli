...
As habilidades do agente permitem que o cybervinci descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O cybervinci pesquisa nesses locais:

- Configuração do projeto: `.cybervinci/skills/<name>/SKILL.md`
- Configuração global: `~/.config/cybervinci/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o cybervinci sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.cybervinci/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/cybervinci/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.cybervinci/skills/git-release/SKILL.md` assim:
...
O cybervinci lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `cybervinci.json`:
...
**Para agentes embutidos** (em `cybervinci.json`):
