...
Existem dois tipos de agentes no cybervinci; agentes primários e subagentes.
...
cybervinci vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
CYBERVINCI vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

CYBERVINCI vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do CYBERVINCI, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `cybervinci.json`:
...
- Global: `~/.config/cybervinci/agents/`
- Por projeto: `.cybervinci/agents/`

```markdown title="~/.config/cybervinci/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o cybervinci usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do cybervinci quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do cybervinci usa o formato `provider/model-id`. Por exemplo, se você estiver usando [OpenCode Zen](/docs/zen), você usaria `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="cybervinci.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="cybervinci.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="cybervinci.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Execute `cybervinci models` para ver uma lista dos modelos disponíveis.
:::
...
```markdown title="~/.config/cybervinci/agents/docs-writer.md"
---
...
```markdown title="~/.config/cybervinci/agents/security-auditor.md"
---
