---
title: Server
description: Interager med cybervinci-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `cybervinci serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en cybervinci-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `cybervinci.local` |
...
Indstil `CYBERVINCI_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `cybervinci`, eller indstil `CYBERVINCI_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `cybervinci serve` og `cybervinci web`.
...
Når du kører `cybervinci` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug cybervinci-serveren til at interagere med cybervinci programmatisk.
:::

Denne arkitektur lader cybervinci understøtte flere klienter og lader dig interagere med cybervinci programmatisk.

Du kan køre `cybervinci serve` for at starte en selvstændig server. Hvis du har
cybervinci TUI kørende, vil `cybervinci serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af CYBERVINCI [IDE](/docs/ide) plugins.
...
CYBERVINCI-serveren viser følgende API'er.
