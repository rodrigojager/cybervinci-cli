...
Der er to typer agenter i CYBERVINCI; primære agenter og subagenter.
...
CYBERVINCI leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
CYBERVINCI leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

CYBERVINCI leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i CYBERVINCI's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `cybervinci.json`-konfigurationsfil:
...
- Globalt: `~/.config/cybervinci/agents/`
- Pr. projekt: `.cybervinci/agents/`
...
Hvis ingen temperatur er angivet, bruger CYBERVINCI modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale CYBERVINCI-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din CYBERVINCI-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [OpenCode Zen](/docs/zen), vil du bruge `opencode/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `cybervinci models` for at se en liste over de tilgængelige modeller.
:::
