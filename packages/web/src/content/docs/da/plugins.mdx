---
title: Plugins
description: Skriv dine egne plugins for at udvide CYBERVINCI.
---

Plugins giver dig mulighed for at udvide CYBERVINCI ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre CYBERVINCIs standardadfærd.
...
- `.cybervinci/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/cybervinci/node_modules/`.
...
1. Global konfiguration (`~/.config/cybervinci/cybervinci.json`)
2. Projektkonfiguration (`cybervinci.json`)
3. Global plugin-mappe (`~/.config/cybervinci/plugins/`)
4. Projekt plugin bibliotek (`.cybervinci/plugins/`)
...
CYBERVINCI kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En cybervinci SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide cybervinci.
...
:::note
Hvis du bruger CYBERVINCI desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå cybervinci i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til cybervinci:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som cybervinci kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for cybervinci sammen med indbyggede værktøjer.
