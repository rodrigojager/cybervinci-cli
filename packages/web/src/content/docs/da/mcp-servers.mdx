...
Du kan tilføje eksterne værktøjer til CYBERVINCI ved hjælp af _Model Context Protocol_ eller MCP. CYBERVINCI understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [CYBERVINCI Config](/docs/config) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/cybervinci`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
CYBERVINCI håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil CYBERVINCI:
...
Hvis serveren kræver godkendelse, vil CYBERVINCI bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `cybervinci mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil CYBERVINCI opbevare tokens sikkert i `~/.local/share/cybervinci/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem CYBERVINCI-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde CYBERVINCI til din Sentry-konto.
