...
Med CYBERVINCI kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger CYBERVINCI vårt eget `cybervinci`-tema.
...
CYBERVINCI kommer med flere innebygde temaer.
...
- Vil at CYBERVINCI skal matche terminalens utseende
...
CYBERVINCI støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/cybervinci/themes/*.json` eller `$XDG_CONFIG_HOME/cybervinci/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.cybervinci/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.cybervinci/themes/*.json`
