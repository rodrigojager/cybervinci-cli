...
Le skill degli agenti permettono a CYBERVINCI di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
CYBERVINCI cerca in queste posizioni:

- Config di progetto: `.cybervinci/skills/<name>/SKILL.md`
- Config globale: `~/.config/cybervinci/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, CYBERVINCI risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.cybervinci/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/cybervinci/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.cybervinci/skills/git-release/SKILL.md` cosi':
...
CYBERVINCI elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `cybervinci.json`:
...
**Per agenti integrati** (in `cybervinci.json`):
