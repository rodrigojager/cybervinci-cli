---
title: Server
description: Interagisci con il server cybervinci via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `cybervinci serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client cybervinci.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `cybervinci.local` |
...
Imposta `CYBERVINCI_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `cybervinci`, oppure imposta `CYBERVINCI_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `cybervinci serve` sia per `cybervinci web`.
...
Quando esegui `cybervinci` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server cybervinci per interagire con cybervinci in modo programmatico.
:::

Questa architettura permette a cybervinci di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `cybervinci serve` per avviare un server standalone. Se la TUI di cybervinci e gia in esecuzione, `cybervinci serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di CYBERVINCI.
...
Il server cybervinci espone le seguenti API.
