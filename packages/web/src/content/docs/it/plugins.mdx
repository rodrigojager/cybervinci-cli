---
title: Plugin
description: Scrivi plugin per estendere CYBERVINCI.
---

I plugin ti permettono di estendere CYBERVINCI agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di CYBERVINCI.
...
- `.cybervinci/plugins/` - plugin a livello progetto
- `~/.config/cybervinci/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/cybervinci/node_modules/`.
...
1. Config globale (`~/.config/cybervinci/cybervinci.json`)
2. Config di progetto (`cybervinci.json`)
3. Directory plugin globale (`~/.config/cybervinci/plugins/`)
4. Directory plugin di progetto (`.cybervinci/plugins/`)
...
CYBERVINCI esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di cybervinci per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere cybervinci.
...
:::note
Se usi l'app desktop di CYBERVINCI, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a cybervinci di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a cybervinci:
...
L'helper `tool` crea uno strumento personalizzato che cybervinci può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in cybervinci insieme agli strumenti integrati.
