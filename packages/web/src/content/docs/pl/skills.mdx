...
Umiejętności agenta pozwalają cybervinci odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
cybervinci przeszukuje te lokalizacje:

- Project config: `.cybervinci/skills/<name>/SKILL.md`
- Global config: `~/.config/cybervinci/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu cybervinci przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.cybervinci/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/cybervinci/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.cybervinci/skills/git-release/SKILL.md` w ten sposób:
...
cybervinci wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `cybervinci.json`:
...
**Dla agentów wbudowanych** (w `cybervinci.json`):
