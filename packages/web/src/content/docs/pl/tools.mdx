...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. CYBERVINCI posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w CYBERVINCI.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `CYBERVINCI_EXPERIMENTAL_LSP_TOOL=true` (lub `CYBERVINCI_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy CYBERVINCI lub gdy zmienna środowiskowa `CYBERVINCI_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu CYBERVINCI:
