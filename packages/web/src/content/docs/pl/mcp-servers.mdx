...
Możesz dodać dodatkowe informacje do cybervinci za pomocą _Model Context Protocol_, w skrócie MCP. cybervinci obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [cybervinci Config](/docs/config) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/cybervinci`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
cybervinci automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, cybervinci:
...
Jeśli serwer wymaga uwierzytelnienia, cybervinci poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `cybervinci mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji cybervinci bezpieczne przechowa tokeny w `~/.local/share/cybervinci/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w cybervinci, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie cybervinci, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć cybervinci z kontem Sentry.
