---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć cybervinci.
---

Wtyczki umożliwiające wyłączenie cybervinci poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania cybervinci.
...
- `.cybervinci/plugins/` - Wtyczki na poziomie projektu
- `~/.config/cybervinci/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/cybervinci/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/cybervinci/cybervinci.json`)
2. Konfiguracja projektu (`cybervinci.json`)
3. Globalny katalog wtyczek (`~/.config/cybervinci/plugins/`)
4. Katalog wtyczek projektu (`.cybervinci/plugins/`)
...
cybervinci uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z cybervinci do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do cybervinci.
...
:::note
Jeśli korzystasz z aplikacji komputerowej cybervinci, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj cybervinci czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do cybervinci:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod cybervinci. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
