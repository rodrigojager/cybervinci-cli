---
title: Serwer
description: Interakcja z serwerem cybervinci za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `cybervinci serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient cybervinci.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `cybervinci.local` |
...
Ustaw `CYBERVINCI_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `cybervinci` lub ustaw `CYBERVINCI_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `cybervinci serve`, jak i `cybervinci web`.
...
Kiedy uruchomisz `cybervinci`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera cybervinci do programowej interakcji z cybervinci.
:::

Ta architektura umożliwia obsługę wielu klientów przez cybervinci i programową interakcję z kodem otwartym.

Możesz uruchomić `cybervinci serve`, aby uruchomić samodzielny serwer. Jeśli masz
cybervinci TUI działa, `cybervinci serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki cybervinci [IDE](/docs/ide).
...
Serwer cybervinci udostępnia następujące interfejsy API.
