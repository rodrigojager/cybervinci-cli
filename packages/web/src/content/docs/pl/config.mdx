---
title: Konfiguracja
description: Ustawienia CYBERVINCI w pliku JSON.
---

Możesz dostosować CYBERVINCI za pomocą pliku konfiguracyjnego JSON.

---

## Format

CYBERVINCI obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
1. **Konfiguracja zdalna** (z `.well-known/cybervinci`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/cybervinci/cybervinci.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `CYBERVINCI_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`cybervinci.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.cybervinci`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `CYBERVINCI_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.cybervinci` i `~/.config/cybervinci` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/cybervinci`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację CYBERVINCI w `~/.config/cybervinci/cybervinci.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/cybervinci/tui.json`.
...
Dodaj `cybervinci.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy CYBERVINCI się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `CYBERVINCI_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `CYBERVINCI_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.cybervinci` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.cybervinci`, więc **może być łączony** z ich zawartością.
...
Użyj `CYBERVINCI_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `cybervinci.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `cybervinci serve` i `cybervinci web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer CYBERVINCI.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `cybervinci.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie CYBERVINCI wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/cybervinci/agents/` lub `.cybervinci/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, CYBERVINCI powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`cybervinci run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/cybervinci/commands/` lub `.cybervinci/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
CYBERVINCI może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy CYBERVINCI nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie CYBERVINCI **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają CYBERVINCI o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.cybervinci/plugins/` lub `~/.config/cybervinci/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie CYBERVINCI tylko do określonych dostawców.
