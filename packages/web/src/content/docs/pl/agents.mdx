...
W CYBERVINCI obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
CYBERVINCI zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
CYBERVINCI ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

CYBERVINCI ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej CYBERVINCI, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `cybervinci.json`:
...
- Globalnie: `~/.config/cybervinci/agents/`
- W projekcie: `.cybervinci/agents/`
...
Jeśli nie zostanie skonfigurowana, CYBERVINCI używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji CYBERVINCI, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji CYBERVINCI w `provider/model-id`. Na przykład, [OpenCode Zen](/docs/zen), użyj `opencode/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `cybervinci models`, aby zobaczyć dostępne modele.
:::
