...
Dzięki cybervinci możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie cybervinci używa naszego własnego motywu `cybervinci`.
...
cybervinci ma kilka wbudowanych motywów.
...
- Chcą, aby CYBERVINCI pasował do wyglądu ich terminala
...
cybervinci obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/cybervinci/themes/*.json` lub `$XDG_CONFIG_HOME/cybervinci/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.cybervinci/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.cybervinci/themes/*.json`
