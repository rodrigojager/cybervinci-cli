...
You can add external tools to CYBERVINCI using the _Model Context Protocol_, or MCP. CYBERVINCI supports both local and remote servers.
...
You can define MCP servers in your [CYBERVINCI Config](/docs/config) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/cybervinci` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
CYBERVINCI automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, CYBERVINCI will:
...
If the server requires authentication, CYBERVINCI will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `cybervinci mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, CYBERVINCI will store the tokens securely in `~/.local/share/cybervinci/mcp-auth.json`.
...
Your MCPs are available as tools in CYBERVINCI, alongside built-in tools. So you can manage them through the CYBERVINCI config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect CYBERVINCI to your Sentry account.
