---
title: LSP Servers
description: CYBERVINCI integrates with your LSP servers.
---

CYBERVINCI can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

CYBERVINCI comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `CYBERVINCI_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and cybervinci opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your cybervinci config.
...
```json title="cybervinci.json" {5-8}
{
  "$schema": "https://opencode.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="cybervinci.json" {5-13}
{
  "$schema": "https://opencode.ai/config.json",
  "lsp": {
    "custom-lsp": {
