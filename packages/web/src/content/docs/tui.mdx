---
title: TUI
description: Using the CYBERVINCI terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CYBERVINCI provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running CYBERVINCI starts the TUI for the current directory.
...
When using the CYBERVINCI TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to CYBERVINCI. Allows you to select from available providers and add their API keys.
...
Exit CYBERVINCI. _Aliases_: `/quit`, `/q`
...
This is separate from `cybervinci.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long CYBERVINCI waits after the leader key. Defaults to `2000`.
...
Use `CYBERVINCI_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `cybervinci.default`.
