...
With CYBERVINCI you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, CYBERVINCI uses our own `cybervinci` theme.
...
CYBERVINCI comes with several built-in themes.
...
- Want CYBERVINCI to match their terminal's appearance
...
CYBERVINCI supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/cybervinci/themes/*.json` or `$XDG_CONFIG_HOME/cybervinci/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.cybervinci/themes/*.json`
4. **Current working directory** - Defined in `./.cybervinci/themes/*.json`
