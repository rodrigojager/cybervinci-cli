...
Puede agregar herramientas externas a CYBERVINCI utilizando el _Model Context Protocol_ o MCP. CYBERVINCI admite servidores locales y remotos.
...
Puede definir servidores MCP en su [CYBERVINCI Config](/docs/config) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/cybervinci`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
CYBERVINCI maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, CYBERVINCI:
...
Si el servidor requiere autenticación, CYBERVINCI le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `cybervinci mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, CYBERVINCI almacenará los tokens de forma segura en `~/.local/share/cybervinci/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en CYBERVINCI, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración CYBERVINCI como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar CYBERVINCI a su cuenta Sentry.
