---
title: Complementos
description: Escriba sus propios complementos para extender CYBERVINCI.
---

Los complementos le permiten extender CYBERVINCI al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de CYBERVINCI.
...
- `.cybervinci/plugins/` - Complementos a nivel de proyecto
- `~/.config/cybervinci/plugins/` - Complementos globales
...
```json title="cybervinci.json"
{
  ...
  "plugin": ["opencode-helicone-session", "cybervinci-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/cybervinci/node_modules/`.
...
1. Configuración global (`~/.config/cybervinci/cybervinci.json`)
2. Configuración del proyecto (`cybervinci.json`)
3. Directorio global de complementos (`~/.config/cybervinci/plugins/`)
4. Directorio de complementos del proyecto (`.cybervinci/plugins/`)
...
```json title=".cybervinci/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
CYBERVINCI ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".cybervinci/plugins/my-plugin.ts"
...
```js title=".cybervinci/plugins/example.js"
...
- `client`: Un cliente SDK cybervinci para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar cybervinci.
...
```js title=".cybervinci/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "CYBERVINCI"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio CYBERVINCI, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que cybervinci lea archivos `.env`:

```javascript title=".cybervinci/plugins/env-protection.js"
...
```javascript title=".cybervinci/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a cybervinci:

```ts title=".cybervinci/plugins/custom-tools.ts"
import { type Plugin, tool } from "@cybervinci-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que cybervinci puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para cybervinci junto con las herramientas integradas.
...
```ts title=".cybervinci/plugins/compaction.ts"
import type { Plugin } from "@cybervinci-ai/plugin"
...
```ts title=".cybervinci/plugins/custom-compaction.ts"
import type { Plugin } from "@cybervinci-ai/plugin"
