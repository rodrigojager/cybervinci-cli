...
Las habilidades del agente permiten a CYBERVINCI descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
CYBERVINCI busca estas ubicaciones:

- Configuración del proyecto: `.cybervinci/skills/<name>/SKILL.md`
- Configuración global: `~/.config/cybervinci/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, CYBERVINCI sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.cybervinci/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/cybervinci/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.cybervinci/skills/git-release/SKILL.md` así:
...
CYBERVINCI enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `cybervinci.json`:
...
**Para agentes integrados** (en `cybervinci.json`):
