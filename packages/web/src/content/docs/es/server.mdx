---
title: Servidor
description: Interactuar con el servidor cybervinci a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `cybervinci serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente cybervinci puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `cybervinci.local` |
...
Configure `CYBERVINCI_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `cybervinci`, o configure `CYBERVINCI_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `cybervinci serve` como a `cybervinci web`.
...
Cuando ejecuta `cybervinci`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor cybervinci para interactuar con cybervinci mediante programación.
:::

Esta arquitectura permite que cybervinci admita múltiples clientes y le permite interactuar con cybervinci mediante programación.

Puede ejecutar `cybervinci serve` para iniciar un servidor independiente. Si tienes el
cybervinci TUI ejecutándose, `cybervinci serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos CYBERVINCI [IDE](/docs/ide).
...
El servidor cybervinci expone las siguientes API.
