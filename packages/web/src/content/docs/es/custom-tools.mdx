...
description: Cree herramientas que LLM pueda llamar en cybervinci.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de cybervinci como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.cybervinci/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/cybervinci/tools/`.
...
```ts title=".cybervinci/tools/database.ts" {1}
...
```ts title=".cybervinci/tools/math.ts"
import { tool } from "@cybervinci-ai/plugin"
...
```ts title=".cybervinci/tools/bash.ts"
import { tool } from "@cybervinci-ai/plugin"
...
```ts title=".cybervinci/tools/project.ts" {8}
import { tool } from "@cybervinci-ai/plugin"
...
```python title=".cybervinci/tools/add.py"
import sys
...
```ts title=".cybervinci/tools/python-add.ts" {10}
import { tool } from "@cybervinci-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".cybervinci/tools/add.py")
