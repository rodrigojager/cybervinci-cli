...
Las herramientas permiten que LLM realice acciones en su código base. CYBERVINCI viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en CYBERVINCI.
...
:::note
Esta herramienta solo está disponible cuando `CYBERVINCI_EXPERIMENTAL_LSP_TOOL=true` (o `CYBERVINCI_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor CYBERVINCI o cuando la variable de entorno `CYBERVINCI_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar CYBERVINCI:

```bash
CYBERVINCI_ENABLE_EXA=1 cybervinci
```
