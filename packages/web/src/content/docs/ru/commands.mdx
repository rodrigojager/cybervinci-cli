...
Создайте `.cybervinci/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию cybervinci или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем cybervinci [config](/docs/config):
...
- Глобальный: `~/.config/cybervinci/commands/`
- Для каждого проекта: `.cybervinci/commands/`
...
cybervinci включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
