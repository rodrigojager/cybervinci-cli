---
title: サーバー
description: HTTP 経由で CYBERVINCI サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`cybervinci serve` コマンドは、CYBERVINCI クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `cybervinci.local` |
...
HTTP 基本認証でサーバーを保護するには、`CYBERVINCI_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `cybervinci` になるか、`CYBERVINCI_SERVER_USERNAME` を設定してオーバーライドします。これは、`cybervinci serve` と `cybervinci web` の両方に当てはまります。
...
`cybervinci` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
CYBERVINCI サーバーを使用して、プログラムで CYBERVINCI と対話します。
:::

`cybervinci serve` を実行してスタンドアロンサーバーを起動できます。すでに CYBERVINCI TUI を実行している場合でも、`cybervinci serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、CYBERVINCI [IDE](/docs/ide) プラグインによって使用されます。
