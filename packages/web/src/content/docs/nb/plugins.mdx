...
description: Skriv dine egne programtillegg for å utvide CYBERVINCI.
---

Plugins lar deg utvide CYBERVINCI ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til CYBERVINCI.
...
- `.cybervinci/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/cybervinci/node_modules/`.
...
1. Global konfigurasjon (`~/.config/cybervinci/cybervinci.json`)
2. Prosjektkonfigurasjon (`cybervinci.json`)
3. Global plugin-katalog (`~/.config/cybervinci/plugins/`)
4. Prosjektpluginkatalog (`.cybervinci/plugins/`)
...
CYBERVINCI kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En CYBERVINCI SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide CYBERVINCI.
...
:::note
Hvis du bruker CYBERVINCI-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre CYBERVINCI fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til CYBERVINCI:
...
`tool`-hjelperen lager et tilpasset verktøy som CYBERVINCI kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for CYBERVINCI sammen med innebygde verktøy.
