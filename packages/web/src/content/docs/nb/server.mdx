---
title: Tjener
description: Samhandle med CYBERVINCI-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `cybervinci serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en CYBERVINCI-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `cybervinci.local` |
...
Sett `CYBERVINCI_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `cybervinci` som standard, eller sett `CYBERVINCI_SERVER_USERNAME` for å overstyre det. Dette gjelder både `cybervinci serve` og `cybervinci web`.
...
Når du kjører `cybervinci` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk CYBERVINCI-serveren til å samhandle med CYBERVINCI programmatisk.
:::

Denne arkitekturen lar CYBERVINCI støtte flere klienter og lar deg samhandle med CYBERVINCI programmatisk.

Du kan kjøre `cybervinci serve` for å starte en frittstående server. Hvis du har
CYBERVINCI TUI kjører, vil `cybervinci serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av CYBERVINCI [IDE](/docs/ide) plugins.
...
cybervinci-serveren viser følgende APIer.
