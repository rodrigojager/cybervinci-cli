---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i CYBERVINCI.
---

import config from "../../../../config.mjs"
export const console = config.console

CYBERVINCI bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i CYBERVINCI-konfigurasjonen.
...
i `~/.local/share/cybervinci/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i CYBERVINCI
konfig.
...
OpenCode Zen er en liste over modeller levert av CYBERVINCI-teamet som har vært
testet og verifisert for å fungere godt med CYBERVINCI. [Finn ut mer](/docs/zen).
...
Det fungerer som alle andre leverandører i CYBERVINCI og er helt valgfritt å bruke.

---

## OpenCode Go

OpenCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av CYBERVINCI-teamet som har vært
testet og verifisert for å fungere godt med CYBERVINCI.
...
Slik bruker du Amazon Bedrock med CYBERVINCI:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører CYBERVINCI:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `cybervinci.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i cybervinci støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere cybervinci til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at CYBERVINCI skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom CYBERVINCI-konfigurasjonen.
...
for å lagre token i CYBERVINCI auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
CYBERVINCI bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse CYBERVINCI
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`cybervinci.json` fil. Det anbefales også å deaktivere øktdeling.
...
Tilpass gjennom `cybervinci.json`:
...
Slik bruker du GitHub Copilot-abonnementet med CYBERVINCI:
...
Slik bruker du Google Vertex AI med CYBERVINCI:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører CYBERVINCI.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom CYBERVINCI, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver CYBERVINCI-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver CYBERVINCI-samtale oppført som en separat økt.
...
Du kan konfigurere CYBERVINCI for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere CYBERVINCI for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere CYBERVINCI for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for CYBERVINCI. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med CYBERVINCI:
...
3. Kopier API-nøkkelen for bruk i CYBERVINCI.
...
6. **Viktig**: Før du bruker skymodeller i CYBERVINCI, må du hente modellinformasjonen lokalt:
...
OpenCode Zen er en liste over testede og verifiserte modeller levert av CYBERVINCI-teamet. [Finn ut mer](/docs/zen).
...
3. Skriv inn CYBERVINCI API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom CYBERVINCI-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom CYBERVINCI-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med CYBERVINCI:
...
Du kan også tilpasse modeller gjennom CYBERVINCI-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med CYBERVINCI. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `cybervinci.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar CYBERVINCI forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `cybervinci auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk CYBERVINCI-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i CYBERVINCI-konfigurasjonen.
