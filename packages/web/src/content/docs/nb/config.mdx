---
title: Konfigurasjon
description: Ved å bruke cybervinci JSON-konfigurasjonen.
---

Du kan konfigurere CYBERVINCI ved å bruke en JSON konfigurasjonsfil.

---

## Format

CYBERVINCI støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Ekstern konfigurasjon** (fra `.well-known/cybervinci`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/cybervinci/cybervinci.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`CYBERVINCI_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`cybervinci.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.cybervinci` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`CYBERVINCI_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.cybervinci` og `~/.config/cybervinci` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/cybervinci`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale CYBERVINCI-konfigurasjon i `~/.config/cybervinci/cybervinci.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `cybervinci.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når CYBERVINCI starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `CYBERVINCI_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `CYBERVINCI_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.cybervinci` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.cybervinci`-katalogene, slik at den **kan overstyre** innstillingene deres.
...
Du kan konfigurere serverinnstillinger for kommandoene `cybervinci serve` og `cybervinci web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage CYBERVINCI-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `cybervinci.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i CYBERVINCI-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver CYBERVINCI å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/cybervinci/agents/` eller `.cybervinci/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil CYBERVINCI falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`cybervinci run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/cybervinci/commands/` eller `.cybervinci/commands/`. [Les mer her](/docs/commands).
...
CYBERVINCI vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater CYBERVINCI alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider CYBERVINCI med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.cybervinci/plugins/` eller `~/.config/cybervinci/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense CYBERVINCI til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
