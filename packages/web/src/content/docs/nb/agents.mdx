...
Det er to typer agenter i CYBERVINCI; primære agenter og underagenter.
...
CYBERVINCI kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
CYBERVINCI kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

CYBERVINCI kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `cybervinci.json`:
...
- Per prosjekt: `.cybervinci/agents/`
...
Hvis ingen temperatur er spesifisert, bruker CYBERVINCI modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale CYBERVINCI-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i CYBERVINCI-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [OpenCode Zen](/docs/zen), vil du bruke `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `cybervinci models` for å se en liste over tilgjengelige modeller.
:::
