...
Du kan legge til eksterne verktøy i CYBERVINCI ved å bruke _Model Context Protocol_ eller MCP. CYBERVINCI støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [CYBERVINCI Config](/docs/config) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="cybervinci.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/cybervinci`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="cybervinci.jsonc" {15}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
CYBERVINCI håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil CYBERVINCI:
...
Hvis serveren krever autentisering, vil CYBERVINCI be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `cybervinci mcp auth <server-name>`.
...
```json title="cybervinci.json" {7-11}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
cybervinci mcp auth my-oauth-server
```
...
```bash
cybervinci mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil CYBERVINCI lagre tokens sikkert i `~/.local/share/cybervinci/mcp-auth.json`.
...
cybervinci mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i CYBERVINCI, sammen med innebygde verktøy. Så du kan administrere dem gjennom CYBERVINCI-konfigurasjonen som alle andre verktøy.
...
```json title="cybervinci.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="cybervinci.json" {4-8}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
cybervinci mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble CYBERVINCI til Sentry-kontoen din.
...
```json title="cybervinci.json" {4-7}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="cybervinci.json" {7-9}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver CYBERVINCI-samtale oppført som en separat økt.
