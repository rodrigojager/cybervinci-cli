---
title: Dodaci
description: Napišite vlastite dodatke za proširenje CYBERVINCI.
---

Dodaci vam omogućavaju da proširite CYBERVINCI spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja CYBERVINCI.
...
- `.cybervinci/plugins/` - Dodaci na nivou projekta
- `~/.config/cybervinci/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/cybervinci/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/cybervinci/cybervinci.json`)
2. Konfiguracija projekta (`cybervinci.json`)
3. Globalni direktorij dodataka (`~/.config/cybervinci/plugins/`)
4. Direktorij dodataka projekta (`.cybervinci/plugins/`)
...
CYBERVINCI pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: CyberVinci SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje CYBERVINCI.
...
Spriječite cybervinci da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u cybervinci:
...
Pomoćnik `tool` kreira prilagođeni alat koji cybervinci može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za cybervinci zajedno sa ugrađenim alatima.
