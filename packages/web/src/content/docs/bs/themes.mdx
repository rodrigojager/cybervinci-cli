...
U CYBERVINCI mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, CYBERVINCI koristi nasu `cybervinci` temu.
...
CYBERVINCI dolazi sa vise ugradenih tema.
...
- Zele da CYBERVINCI odgovara izgledu njihovog terminala
...
CYBERVINCI podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/cybervinci/themes/*.json` ili `$XDG_CONFIG_HOME/cybervinci/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.cybervinci/themes/*.json`
4. **Trenutni radni direktorij** - `./.cybervinci/themes/*.json`
