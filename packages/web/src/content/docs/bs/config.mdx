---
title: Konfiguracija
description: Korištenje CYBERVINCI JSON konfiguracije.
---

Možete konfigurirati CYBERVINCI koristeći JSON konfiguracijski fajl.

---

## Format

CYBERVINCI podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/cybervinci`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/cybervinci/cybervinci.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`CYBERVINCI_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`cybervinci.json` u projektu) - postavke specifične za projekat
5. **`.cybervinci` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`CYBERVINCI_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.cybervinci` i `~/.config/cybervinci` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/cybervinci` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu CYBERVINCI konfiguraciju u `~/.config/cybervinci/cybervinci.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/cybervinci/tui.json`.
...
Dodajte `cybervinci.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se CYBERVINCI pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `CYBERVINCI_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `CYBERVINCI_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.cybervinci` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.cybervinci`, tako da **može nadjačati** njihove postavke.
...
Koristite `CYBERVINCI_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `cybervinci.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `cybervinci serve` i `cybervinci web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš CYBERVINCI server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `cybervinci.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj CYBERVINCI konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, CYBERVINCI pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/cybervinci/agents/` ili `.cybervinci/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, CYBERVINCI će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`cybervinci run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/cybervinci/commands/` ili `.cybervinci/commands/`. [Saznajte više ovdje](/docs/commands).
...
CYBERVINCI će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, CYBERVINCI **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju CYBERVINCI sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.cybervinci/plugins/` ili `~/.config/cybervinci/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite CYBERVINCI da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
