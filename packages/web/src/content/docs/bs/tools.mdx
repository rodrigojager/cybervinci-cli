...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. CYBERVINCI dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u CYBERVINCI.
...
:::note
Ovaj alat je dostupan samo kada je `CYBERVINCI_EXPERIMENTAL_LSP_TOOL=true` (ili `CYBERVINCI_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz CYBERVINCI provajdera ili kada je varijabla `CYBERVINCI_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju CYBERVINCI:
