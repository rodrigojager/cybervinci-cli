...
Možete dodati vanjske alate u CYBERVINCI koristeći _Model Context Protocol_, ili MCP. CYBERVINCI podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [CYBERVINCI Config](/docs/config) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/cybervinci`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
CYBERVINCI automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, CYBERVINCI će:
...
Ako server zahtijeva autentifikaciju, CYBERVINCI će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `cybervinci mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, CYBERVINCI sigurno čuva tokene u `~/.local/share/cybervinci/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u CYBERVINCI, zajedno s ugrađenim alatima. Možete njima upravljati kroz CYBERVINCI konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete CYBERVINCI sa vašim Sentry nalogom.
