---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u CYBERVINCI.
---

import config from "../../../../config.mjs"
export const console = config.console

CYBERVINCI koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj CYBERVINCI konfiguraciji.
...
u `~/.local/share/cybervinci/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem CYBERVINCI
config.
...
OpenCode Zen je lista modela koje je obezbedio CYBERVINCI tim koji su bili
testirano i potvrđeno da dobro radi sa CYBERVINCI. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OpenCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite cybervinci i idite na [opencode.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u CYBERVINCI i potpuno je opcionalan za korištenje.

---

## OpenCode Go

OpenCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža CYBERVINCI tim i koji su testirani i verificirani da dobro rade s CYBERVINCI-om.
...
Da biste koristili Amazon Bedrock s CYBERVINCI:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete cybervinci:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `cybervinci.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u CYBERVINCI.
:::
...
Možete konfigurirati cybervinci za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi cybervinci ispravno radio.
   :::
...
Također možete dodati modele kroz svoju cybervinci konfiguraciju.
...
da pohrani token u cybervinci auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
CYBERVINCI koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate CYBERVINCI
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`cybervinci.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `cybervinci.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s cybervinci:
...
Za korištenje Google Vertex AI s CYBERVINCI:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće cybervinci.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem cybervincia, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki CYBERVINCI razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki CYBERVINCI razgovor naveden kao zasebna sesija.
...
Možete konfigurirati cybervinci za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati cybervinci za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati cybervinci za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za CYBERVINCI. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s CYBERVINCI:
...
3. Kopirajte API ključ za korištenje u CYBERVINCI.
...
6. **Važno**: Prije upotrebe modela oblaka u CYBERVINCI, morate lokalno povući informacije o modelu:
...
OpenCode Zen je lista testiranih i verifikovanih modela koju je obezbedio CYBERVINCI tim. [Saznajte više](/docs/zen).
...
3. Unesite svoj CYBERVINCI API ključ.
...
Također možete dodati dodatne modele putem vaše cybervinci konfiguracije.
...
5. Također ih možete prilagoditi putem vaše cybervinci konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa CyberVinciom:
...
Također možete prilagoditi modele kroz svoju cybervinci konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s cybervinci-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `cybervinci.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju CYBERVINCI da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `cybervinci auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite CYBERVINCI konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj cybervinci konfiguraciji.
