...
Agent skills omogucavaju CYBERVINCI da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
CYBERVINCI pretrazuje ove lokacije:

- Konfiguracija projekta: `.cybervinci/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/cybervinci/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, CYBERVINCI ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.cybervinci/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/cybervinci/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.cybervinci/skills/git-release/SKILL.md` ovako:
...
CYBERVINCI navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `cybervinci.json`:
...
**Za ugradene agente** (u `cybervinci.json`):
