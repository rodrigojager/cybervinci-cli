...
Postoje dvije vrste agenata u CYBERVINCI: primarni agenti i podagenti.
...
CYBERVINCI dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
CYBERVINCI dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

CYBERVINCI dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u CYBERVINCI-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `cybervinci.json`:
...
- Globalno: `~/.config/cybervinci/agents/`
- Po projektu: `.cybervinci/agents/`
...
Ako temperatura nije navedena, CYBERVINCI koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu CYBERVINCI konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj CYBERVINCI konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [OpenCode Zen](/docs/zen), koristili biste `opencode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `cybervinci models` da vidite listu dostupnih modela.
:::
