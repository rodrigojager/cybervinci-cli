---
title: Server
description: Komunicirajte s cybervinci serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `cybervinci serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji cybervinci klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `cybervinci.local` |
...
Postavite `CYBERVINCI_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `cybervinci`, ili postavite `CYBERVINCI_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `cybervinci serve` i za `cybervinci web`.
...
Kada pokrenete `cybervinci`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite cybervinci server za programsku interakciju sa cybervinci-om.
:::

Ova arhitektura omogucava cybervinci podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `cybervinci serve` da startate standalone server. Ako je cybervinci TUI vec pokrenut, `cybervinci serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste CYBERVINCI [IDE](/docs/ide) pluginovi.
...
cybervinci server izlaže sljedece API-je.
