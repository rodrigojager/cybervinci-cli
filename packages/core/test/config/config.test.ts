...
import { ConfigProvider } from "@cybervinci-ai/core/config/provider"
...
import { ConfigMigrateV1 } from "@cybervinci-ai/core/v1/config/migrate"
...
import { Location } from "@cybervinci-ai/core/location"
import { Policy } from "@cybervinci-ai/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads cybervinci JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "cybervinci.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "cybervinci.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "cybervinci.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "cybervinci.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "cybervinci.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "cybervinci.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "cybervinci.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .cybervinci configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".cybervinci"), { recursive: true })
            await fs.mkdir(path.join(directory, ".cybervinci"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "cybervinci.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "cybervinci.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "cybervinci.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "cybervinci.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "cybervinci.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".cybervinci", "cybervinci.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".cybervinci", "cybervinci.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".cybervinci")),
              AbsolutePath.make(path.join(directory, ".cybervinci")),
            ])
