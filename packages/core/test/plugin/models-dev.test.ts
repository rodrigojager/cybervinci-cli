...
import { ModelsDevPlugin } from "@cybervinci-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.CYBERVINCI_MODELS_PATH,
          disabled: Flag.CYBERVINCI_DISABLE_MODELS_FETCH,
        }
        Flag.CYBERVINCI_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.CYBERVINCI_MODELS_PATH = previous.path
          Flag.CYBERVINCI_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
