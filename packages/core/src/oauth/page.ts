...
// etc.).
//
// The visual language mirrors the CYBERVINCI app: the design tokens are a curated
// subset of the OC-2 semantic tokens in `packages/ui/src/styles/theme.css`.
// The callback uses a dedicated textual wordmark so upstream OpenCode geometry
// cannot leak into CyberVinci authentication screens.
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `CYBERVINCI is now connected to ${escapeHtml(provider)}.` : "CYBERVINCI is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `CYBERVINCI couldn't finish connecting to ${escapeHtml(provider)}.`
        : "CYBERVINCI couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from CYBERVINCI.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · CYBERVINCI</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("CYBERVINCI couldn't finish connecting to "+PROVIDER+"."):"CYBERVINCI couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from CYBERVINCI."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("CYBERVINCI is now connected to "+PROVIDER+"."):"CYBERVINCI is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
const WORDMARK = `<span class="wordmark">CYBERVINCI</span>`
