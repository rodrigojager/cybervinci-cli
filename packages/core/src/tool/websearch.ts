...
import { ToolFailure } from "@cybervinci-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@cybervinci/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.CYBERVINCI_WEBSEARCH_PROVIDER === "exa" || process.env.CYBERVINCI_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.CYBERVINCI_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("CYBERVINCI_EXPERIMENTAL") || truthy("CYBERVINCI_ENABLE_EXA") || truthy("CYBERVINCI_EXPERIMENTAL_EXA"),
    enableParallel: truthy("CYBERVINCI_ENABLE_PARALLEL") || truthy("CYBERVINCI_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `cybervinci/${InstallationVersion}`,
