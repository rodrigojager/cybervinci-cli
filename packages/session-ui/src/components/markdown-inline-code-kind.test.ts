...
describe("inlineCodeKind", () => {
  test("leaves code expressions as normal inline code", () => {
    ...
    expect(inlineCodeKind(`@cybervinci-ai/app <StatusPopover />)`)).toBeUndefined()
  ...
  test("detects file and directory paths", () => {
    ...
    expect(inlineCodeKind(`~/.config/cybervinci`)).toBe("path")
    expect(inlineCodeKind(`@cybervinci-ai/app`)).toBe("path")
    expect(inlineCodeKind(`session/status`)).toBe("path")
  })

  test("detects urls", () => {
    expect(inlineCodeKind(`https://docs.cybervinci.invalid`)).toBe("url")
    expect(inlineCodeKind(`http://localhost:4444`)).toBe("url")
    expect(inlineCodeKind(`file:///tmp/cybervinci`)).toBeUndefined()
    expect(inlineCodeKind(`ftp://docs.cybervinci.invalid`)).toBeUndefined()
  })
})
