...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("cybervinci-find")
  api.delete("cybervinci-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("cybervinci-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("cybervinci-find", new Highlight(...rest))
    return true
  }
