...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("cybervinci://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("cybervinci://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("cybervinci://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("cybervinci://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("cybervinci://open-project")).toBeUndefined()
    expect(parseDeepLink("cybervinci://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "cybervinci://open-project?directory=/a",
      "cybervinci://other?directory=/b",
      "cybervinci://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("cybervinci://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("cybervinci://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("cybervinci://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("cybervinci://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "cybervinci://new-session?directory=/a",
      "cybervinci://open-project?directory=/b",
      "cybervinci://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __CYBERVINCI__: {
        deepLinks: ["cybervinci://open-project?directory=/a"],
      },
    } as unknown as Window & { __CYBERVINCI__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["cybervinci://open-project?directory=/a"])
