...
import type { retry } from "@cybervinci-ai/core/util/retry"
import type { OpenCodeEvent, SessionApi } from "@cybervinci-ai/client/promise"
import type { Message, CyberVinciClient, Part, Session } from "@cybervinci-ai/sdk/v2/client"
...
function messageClient(...responses: Array<MessageResponse | Promise<MessageResponse>>) {
  ...
  } as unknown as CyberVinciClient
...
describe("server session", () => {
  ...
  test("extends a current page to include the user for split assistant turns", async () => {
    ...
    const store = createServerSession({} as CyberVinciClient, {} as SessionApi, messageApi)
    store.remember(session("root"))
