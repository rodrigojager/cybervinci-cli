...
import { Tag } from "@cybervinci-ai/ui/v2/badge-v2"
...
import { wslCyberVinciAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.cybervinciChecks[item.config.distro]
          const cybervinciAction = () => wslCyberVinciAction(check())
          const busy = () => wsl.data?.job?.kind === "install-cybervinci" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={cybervinciAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installCyberVinci(item.config.distro))}
                    >
