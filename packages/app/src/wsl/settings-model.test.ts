...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when CYBERVINCI needs attention", () => {
    expect(wslCyberVinciAction(undefined)).toBeUndefined()
    expect(
      wslCyberVinciAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("wsl.onboarding.installCyberVinci")
    expect(
      wslCyberVinciAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/cybervinci",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("wsl.onboarding.updateCyberVinci")
    expect(
      wslCyberVinciAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/cybervinci",
  ...
  test("does not report ready when CYBERVINCI is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        cybervinciChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.cybervinci/bin/cybervinci",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installCyberVinci" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-cybervinci")
  })
