...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-cybervinci" | "add" | null
  loading: boolean
  width: string | null
}
...
export function addServerViewModel(input: {
  ...
  const cybervinciCheck = selectedDistro ? (state?.cybervinciChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.cybervinciChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateCyberVinci" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.cybervinciMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installCyberVinci" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  cybervinciCheck: WslCyberVinciCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const probingCyberVinci =
    probingSelected ||
    (ready &&
      (!input.cybervinciCheck ||
        (!!input.selectedDistro &&
          input.state?.job?.kind === "probe-addable" &&
          input.state.job.distros.includes(input.selectedDistro))))
  const installingCyberVinci =
    input.state?.job?.kind === "install-cybervinci" && input.state.job.distro === input.selectedDistro
  if (!ready || probingCyberVinci) {
    return {
      variant: "contrast",
  ...
  if (!addServerCyberVinciReady(input.cybervinciCheck)) {
    const update = !!input.cybervinciCheck?.resolvedPath && input.cybervinciCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingCyberVinci
        ? { key: "wsl.onboarding.updatingCyberVinci" }
        : update
          ? { key: "wsl.onboarding.updateCyberVinci" }
          : { key: "wsl.onboarding.installCyberVinci" },
      disabled: !!input.state?.job || input.adding,
      action: "install-cybervinci",
      loading: installingCyberVinci,
...
function addServerCyberVinciReady(check: WslCyberVinciCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.cybervinciChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.cybervinciChecks[item.name]) return [`cybervinci:${item.name}`]
    return []
  })
