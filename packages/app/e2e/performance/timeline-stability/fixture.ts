...
import { SessionStatusEvent } from "@cybervinci-ai/schema/session-status-event"
import { SessionV1 } from "@cybervinci-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/CYBERVINCI/TimelineStability"
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("cybervinci.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
