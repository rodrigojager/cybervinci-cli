...
import { getCyberVinciModeStack, CYBERVINCI_BASE_MODE, CyberVinciKeymapProvider, registerCyberVinciKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerCyberVinciKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when cybervinci mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getCyberVinciModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getCyberVinciModeStack(keymap).push("autocomplete")
