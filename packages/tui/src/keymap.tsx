...
export const CYBERVINCI_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const CYBERVINCI_MODE_KEY = "cybervinci.mode"

export const CyberVinciKeymapProvider = KeymapProvider
export const useCyberVinciKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type CyberVinciModeStack = ReturnType<typeof createCyberVinciModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, CyberVinciModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createCyberVinciModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(CYBERVINCI_MODE_KEY, CYBERVINCI_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(CYBERVINCI_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(CYBERVINCI_MODE_KEY, stack.at(-1)?.mode ?? CYBERVINCI_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? CYBERVINCI_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(CYBERVINCI_MODE_KEY, undefined)
...
export function useCyberVinciModeStack() {
  return getCyberVinciModeStack(useCyberVinciKeymap())
}

export function getCyberVinciModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("CyberVinci mode stack is not registered for this keymap")
  return value
}
...
export function registerCyberVinciKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createCyberVinciModeStack(keymap)
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  const keymap = useCyberVinciKeymap()
