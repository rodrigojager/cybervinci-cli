import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@cybervinci-ai/plugin/tui"
...
import type { useCyberVinciKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useCyberVinciKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getCyberVinciModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getCyberVinciModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
