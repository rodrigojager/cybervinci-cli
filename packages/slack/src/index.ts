...
import { createCyberVinci, type ToolPart } from "@cybervinci-ai/sdk"
...
console.log("🚀 Starting cybervinci server...")
const cybervinci = await createCyberVinci({
  port: 0,
})
console.log("✅ CyberVinci server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await cybervinci.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new cybervinci session...")
    const { client, server } = cybervinci
    ...
    console.log("✅ Created cybervinci session:", createResult.data.id)
  ...
  console.log("📝 Sending to cybervinci:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 CyberVinci response:", JSON.stringify(result, null, 2))
