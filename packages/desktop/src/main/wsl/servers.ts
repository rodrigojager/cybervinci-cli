...
type WslServersControllerOptions = {
  ...
  resolveCyberVinci?: typeof resolveWslCyberVinci
...
export function createWslServersController(
  ...
  const setCyberVinciCheck = (distro: string, check: WslCyberVinciCheck) => {
    setState({
      cybervinciChecks: {
        ...state.cybervinciChecks,
        [distro]: check,
      },
    })
  }

  const checkCyberVinci = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveCyberVinci ?? resolveWslCyberVinci)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return cybervinciCheck(distro, resolved, version, appVersion)
  }

  const refreshCyberVinciCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setCyberVinciCheck(distro, await checkCyberVinci(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const cybervinciChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.cybervinciChecks[distro])
        .map(async (distro) => [distro, await checkCyberVinci(distro, opts)] as const),
    )
    if (cybervinciChecks.length) {
      setState({ cybervinciChecks: { ...state.cybervinciChecks, ...Object.fromEntries(cybervinciChecks) } })
    }
  }
  ...
  const refreshCyberVinciCheckBackground = (id: string, distro: string) => {
    void checkCyberVinci(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setCyberVinciCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl cybervinci check failed", { id, distro, message })
      })
  }

  const refreshCyberVinciChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkCyberVinci(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setCyberVinciCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl cybervinci check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshCyberVinciCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installCyberVinci(name: string) {
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.cybervinciChecks, distro) : {}),
      })
    },
