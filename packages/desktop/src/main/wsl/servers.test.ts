...
let releaseCyberVinciResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectCyberVinciVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectCyberVinciVersion("1.14.35", "1.16.2")).toThrow(
    "CYBERVINCI update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating CYBERVINCI", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.cybervinci/bin/cybervinci",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, cybervinciChecks: {} })
})
...
test("ignores stale background CYBERVINCI checks after removing a WSL server", async () => {
  persistedServers = []
  releaseCyberVinciResolve = undefined
  ...
  await waitFor(() => !!releaseCyberVinciResolve)
  await controller.removeServer("wsl:Debian")
  releaseCyberVinciResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().cybervinciChecks).toEqual({})
})

test("ignores stale startup CYBERVINCI checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking CYBERVINCI", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const cybervinci: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveCyberVinci: async (distro) => {
      cybervinci.push(distro)
      return "/home/me/.cybervinci/bin/cybervinci"
    },
  })
  ...
  expect(cybervinci).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(cybervinci).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().cybervinciChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check CYBERVINCI in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const cybervinci: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(cybervinci).toEqual(["Debian"])
  expect(Object.keys(controller.getState().cybervinciChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolveCyberVinci: async () => {
      await new Promise<void>((resolve) => {
        releaseCyberVinciResolve = resolve
      })
      return "/home/me/.cybervinci/bin/cybervinci"
    },
  }
