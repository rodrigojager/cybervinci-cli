...
import { type WslCommandLine, resolveWslCyberVinci, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const cybervinci = await resolveWslCyberVinci(distro)
  if (!cybervinci) throw new Error(nativeT("desktop.wsl.error.cybervinciNotInstalled", { distro }))

  const port = await allocatePort()
  const password = randomUUID()
  const username = "cybervinci"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export CYBERVINCI_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export CYBERVINCI_CLIENT=desktop",
    `export CYBERVINCI_SERVER_USERNAME=${shellEscape(username)}`,
    `export CYBERVINCI_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(cybervinci)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
