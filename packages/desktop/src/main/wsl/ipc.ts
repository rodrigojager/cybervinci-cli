...
export function registerWslIpcHandlers(controller: WslServersController) {
  ...
  ipcMain.handle("wsl-servers-install-cybervinci", (_event: IpcMainInvokeEvent, name: string) =>
    controller.installCyberVinci(requireWslIpcString("distro", name)),
  )
...
function registerUnavailableWslIpcHandlers() {
  ...
  ipcMain.handle("wsl-servers-install-cybervinci", unavailable)
