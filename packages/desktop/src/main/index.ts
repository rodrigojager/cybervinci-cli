...
const APP_NAMES: Record<string, string> = {
  dev: "CYBERVINCI Dev",
  beta: "CYBERVINCI Beta",
  prod: "CYBERVINCI",
}
const APP_IDS: Record<string, string> = {
  dev: "ai.cybervinci.desktop.dev",
  beta: "ai.cybervinci.desktop.beta",
  prod: "ai.cybervinci.desktop",
}
const TEST_ONBOARDING = process.env.CYBERVINCI_TEST_ONBOARDING === "1"
const SIDECAR_VERSION = process.env.CYBERVINCI_SIDECAR_V2 === "1" ? "v2" : "v1"
...
const main = Effect.gen(function* () {
  ...
  process.env.CYBERVINCI_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.cybervinci.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `cybervinci-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "CYBERVINCI Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter(
      (arg: string) => arg.startsWith("cybervinci://") || arg.startsWith("opencode://"),
    )
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("cybervinci")
  registerRendererProtocol()
  setDockIcon()
  ...
  const loadingTask = yield* Effect.gen(function* () {
    ...
    const port = yield* Effect.gen(function* () {
      const fromEnv = process.env.CYBERVINCI_PORT
      if (fromEnv) {
