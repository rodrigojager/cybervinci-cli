...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.cybervinci.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `cybervinci.settings.dat` is special: it maps to the `cybervinci.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"cybervinci.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // cybervinci.settings.dat → the electron settings store ("cybervinci.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "cybervinci.global.dat").
  const storeName = filename === "cybervinci.settings.dat" ? "cybervinci.settings" : filename
