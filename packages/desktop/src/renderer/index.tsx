...
import {
  ...
  useCommand,
  useWslServers,
} from "@cybervinci-ai/app"
import type { UpdaterState } from "@cybervinci-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@cybervinci-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.CYBERVINCI_CHANNEL === "prod" &&
...
const deepLinkEvent = "cybervinci:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__CYBERVINCI__ ??= {}
  const pending = window.__CYBERVINCI__.deepLinks ?? []
  window.__CYBERVINCI__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `cybervinci.desktop.window.${windowID}.last-active-url`
}
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("cybervinci.global.dat").getItem("language")
