...
import { createCyberVinci } from "@cybervinci-ai/sdk"
...
`)
  ...
  const cybervinci = await createCyberVinci({ port: 0 })
  ...
  try {
    ...
    const session = await cybervinci.client.session.create()
    const result = await cybervinci.client.session
      .prompt({
