...
import { createCyberVinciClient } from "@cybervinci-ai/sdk"
...
const { client, server } = createCyberVinci()
...
try {
  ...
  await assertCyberVinciConnected()
...
function createCyberVinci() {
  const host = "127.0.0.1"
  const port = 4096
  const url = `http://${host}:${port}`
  const proc = spawn(`cybervinci`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createCyberVinciClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/cybervinci|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/cybervinci` or `/oc`")
  }
}
...
async function assertCyberVinciConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to cybervinci server")
  }
}
...
async function getAccessToken() {
  ...
  const configuredBaseURL = process.env["OIDC_BASE_URL"]
  if (!configuredBaseURL) {
    throw new Error("OIDC_BASE_URL is required when TOKEN is not provided.")
  }
  const oidcBaseURL = configuredBaseURL.replace(/\/+$/, "")
  ...
  if (isMock()) {
    ...
    const oidcToken = await core.getIDToken("cybervinci-github-action")
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/cybervinci" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/cybervinci") || body.includes("/oc")) {
      if (reviewContext) {
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `cybervinci/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const shareUrl = shareId ? `[cybervinci session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
